...
if(isset($_SESSION['__uid'])){
  ...
  if($__uid==0){
    ...
    if($mode==4){
      //save new value
      if($permName==""){
        $errorMessage.="Must set name for the permission<br>";
        $mode=1;
      }

      if($mode==4){
        $q="insert into  permissions(permName) values(?)";
        if ($stmt = mysqli_prepare($dbc, $q)) {
          mysqli_stmt_bind_param($stmt, "s", $permName);
    ...
    if($mode==1){
      ...
      echo "<input class='input-field' type='text' placeholder='Permission Name' name='permName'";
      if(isset($permName))
        echo " value='$permName'";
      echo ">";
      echo "</div>";
    ...
    if($mode==5){
      //save new value
      if($permName==""){
        $errorMessage.="Must set a value for the permission<br>";
        $mode=2;
      }
      if($mode==5){
        $q="update  permissions set  permName=? where permId=?";
        if ($stmt = mysqli_prepare($dbc, $q)) {
          mysqli_stmt_bind_param($stmt, "si", $permName,$permId);
          mysqli_stmt_execute($stmt);
          mysqli_stmt_close($stmt);
        }
      }
    ...
    if($mode==2){
      $showList=false;
      $q="select permName from  permissions where permId=?";
      if ($stmt = mysqli_prepare($dbc, $q)) {
        mysqli_stmt_bind_param($stmt, "i", $permId);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $permName);
      ...
      echo "<div class='input-container'>";
      echo "<input class='input-field' type='text' placeholder='System Value Name' name='permName'";
      if(isset($permName))
        echo " value='$permName'";
      echo ">";
      echo "</div>";
      echo "<input type='hidden' name='permId' value='$permId'>";
    ...
    if($mode==3){
      ...
      echo "<div style='text-align: center;'>Are you sure you want to delete $permName ?...</div><br>";
      echo "<input type='hidden' name='permId' value='$permId'>";
    ...
    if($mode==6){
      $q="delete from permissions where permId=?";
      if ($stmt = mysqli_prepare($dbc, $q)) {
        mysqli_stmt_bind_param($stmt, "i", $permId);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    ...
    if($showList){
      $q="select * from permissions order by permName";
      ...
      if($r){
        ...
        echo "<th>Name</th><th></th><th style=\"width:300px;text-align: center;\"></th></tr>";
        while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
          foreach($row as $key => $value)
            $$key=$value;
          echo "<tr><td>$permName</td><td></td>";
          echo "<td><form method='post'><input type='hidden' name='permId' value='$permId'><button type='submit' class='edtBtn' name='mode' value='2'>Edit</button> <button type='submit' class='delBtn' name='mode' value='3'>Delete</button></form></td>";
          echo "</tr>";
        }
        echo "</table>";

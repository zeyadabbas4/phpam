  ...
  if($logged){
    ...
    if($mode==5){
      ...
      if($errorMessage == ""){
        $newid=-1;
        //save master
        $q="INSERT INTO companies(cmpName,cmpSector,cmpAddress,cmpTels,cmpFax,cmpEmail,cmpSite,cmpNotes) VALUES (?,?,?,?,?,?,?,?);";
        if($stmt = mysqli_prepare($dbc, $q)){
          mysqli_stmt_bind_param($stmt, "sissssss", $cmpName,$cmpSector,$cmpAddress,$cmpTels,$cmpFax,$cmpEmail,$cmpSite,$cmpNotes);
    ...
    if($mode==6){
      //save edited value
      if($errorMessage==""){
        $q="UPDATE companies SET cmpName=?,cmpSector=?,cmpAddress=?,cmpTels=?,cmpFax=?,cmpEmail=?,cmpSite=?,cmpNotes=? WHERE cmpId=?";
        if ($stmt = mysqli_prepare($dbc, $q)){
          mysqli_stmt_bind_param($stmt, "sissssssi", $cmpName,$cmpSector,$cmpAddress,$cmpTels,$cmpFax,$cmpEmail,$cmpSite,$cmpNotes,$cmpId);
          if(!mysqli_stmt_execute($stmt)){
            $errorMessage.= "Error saving data! ...<br>";
          }
          mysqli_stmt_close($stmt);
    ...
    if($mode==2){
      $showList=false;
      if($errorMessage ==""){
        $q="SELECT cmpName,cmpSector,cmpAddress,cmpTels,cmpFax,cmpEmail,cmpSite,cmpNotes FROM companies WHERE cmpId=?";
        if ($stmt = mysqli_prepare($dbc, $q)) {
          mysqli_stmt_bind_param($stmt, "i", $cmpId);
          mysqli_stmt_execute($stmt);
          mysqli_stmt_bind_result($stmt, $cmpName,$cmpSector,$cmpAddress,$cmpTels,$cmpFax,$cmpEmail,$cmpSite,$cmpNotes);
    ...
    if($mode==3){
      ...
      $showList=false;
      if($errorMessage ==""){
        $q="SELECT cmpName, cmpFax, cmpAddress, cmpEmail, cmpTels, cmpSite, cmpNotes, Secname FROM companies INNER JOIN Sectors ON cmpSector = Secid WHERE cmpId=?";
        if ($stmt = mysqli_prepare($dbc, $q)) {
          mysqli_stmt_bind_param($stmt, "i", $cmpId);
          mysqli_stmt_execute($stmt);
          mysqli_stmt_bind_result($stmt, $cmpName, $cmpFax, $cmpAddress, $cmpEmail, $cmpTels, $cmpSite, $cmpNotes, $Secname);
          mysqli_stmt_fetch($stmt);
          mysqli_stmt_close($stmt);
        }  
      }
    ...
    if($showList){
      ...
      if($r){
        ...
        while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
          ...
          $rowNo++;
          echo "<tr><td>$rowNo - $cmpName - $Secname</td>";

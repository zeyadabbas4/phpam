...
if(isset($_SESSION['uid'])){
	...
	$pagesize=50;
	//Standard Queries
	$countQuery="SELECT count(*) as reccount FROM tasks";
	$listQueryBase="SELECT * FROM tasks where task_user=$usrid ORDER BY task_assign_date desc";
	//standard Columns
	$showEdit=true;
	$showVeiw=true;
	$showDelete=true;
...
//	if(checkuserscreen($usrid,$scrid)){
	...
	if($showtable){
		...
		if($r){
			//column headers - Don't Edit
			echo "<tr style='font-size:medium;font-weight:bold;background-color:$tblheadbg;'>";
                        for($i=0;$i<2;$i++)
                        echo "<td>&nbsp; ". $lcol[$i] ."&nbsp;</td>";
                        if($showEdit) $colcount++;
                        if($showDelete) $colcount++;
                        if($showVeiw) $colcount++;
			echo str_repeat("<td>&nbsp;</td>",$colcount);		//empty cells for action buttons
			echo "</tr>";

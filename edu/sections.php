...
// form variables
//***********************************************************************************************
$masterTable="EduSections";
$noFields=2;
$fieldNames[0]="SecId";
$fieldNames[1]="SecName";
$fieldDataTypes[0]="int";
$fieldDataTypes[1]="str";
$fieldInputTypes[0]="";
$fieldInputTypes[1]="text";
$fieldRadioValues[5]=array(0,1);
$fieldRadioLabels[5]=array("مصروف","إيراد");
$fieldDataSource[6]="select * from source table";
$fieldSelectValueField[6]="cityName";
$fieldSelectLabelField[6]="cityId";
$fieldLabels[0]="رقم القسم";
$fieldLabels[1]="اسم القسم";
$fieldDefaultValues[0]="";
$fieldDefaultValues[1]="";
$fieldsRequired[0]=false;
$fieldsRequired[1]=true;
$insertFields=array(1);                   //Fields in the insert query
$updateFields=array(1);                       //fields in the update query
$viewFields=array(0,1);                       //fields in the view screen
$masterKey=array(0);                              //table key field
$listFields=array(1);                         //fields in the list
$orderList=true;                                  //user order clause to order the list
$listOrderFields=array(1);                        //fields in order clause
...
$showKeyInAdd=false;
$addFormLabel="جديد";
$addFormFields=array(1);                      //fields in the add form
$editFormLabel="تعديل";
$editFormFields=array(1);                     //fields in the edit form
$saveLabel="حفظ";
$cancelLabel="تراجع";
$okLabel="حسنا";
$showAddButton=true;
        ...
        if($showField){
          ...
          switch ($fieldInputType){
            ...
            case "radio":
              echo "<tr><td style='width: 225px;'>$fieldLabel:</td><td colspan='2'>";
              echo "<div class='input-container'>";
              $radioValues=$fieldRadioValues[$i];
              $radioLabels=$fieldRadioLabels[$i];
              foreach($radioValues as $key => $value){
                $optionsArray[$key]['label']=$radioLabels[$key];
                $optionsArray[$key]['value']=$value;
              }
              radioGroup($optionsArray,$fieldName,"H",$fieldDefaultValue);
              echo "</div></td></tr>";  
              break;
            case "select":
              echo "<tr><td style='width: 225px;'>$fieldLabel:</td><td colspan='2'>";
              echo "<div class='input-container'>";
              $sql=$fieldDataSource[$i];
              $valueField=$fieldSelectValueField[$i];
              $labelField=$fieldSelectLabelField[$i];
              $controlName=$fieldName;
              dropdownlist($dbc,$sql,$valueField,$labelField,$controlName,$fieldDefaultValue);
              echo "</div></td></tr>";  
            break;
            }
        }
...
//*****************************************************************************************
    if($mode=="view"){
      ...
      foreach($viewFields as $value){
        $fieldInputType=$fieldInputTypes[$value];
        $fieldLabel=$fieldLabels[$value];
        $fieldName=$fieldNames[$value];
        if($fieldInputType=="radio"){
          $radioLabels=$fieldRadioLabels[$value];
          echo "<tr><td style='width: 225px;'>$fieldLabel:</td><td>";
          echo "<div class='input-container'>".$radioLabels[$$fieldName]."</div></td></tr>";    
        }else{
          echo "<tr><td style='width: 225px;'>$fieldLabel:</td><td>";
...
//*****************************************************************************************
    if($showList){
      ...
      $listQuery="select $fieldList from $masterTable $whereClause $orderClause";
      $q=$listQuery;
      $r=mysqli_query($dbc,$q);
      if($r){
        if($showAddButton){

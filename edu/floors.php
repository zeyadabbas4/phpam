...
if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
    $q="SELECT floorDescription FROM floors WHERE floorId=?";
    if($stmt=mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt, "i", $floorId)){
            if(mysqli_stmt_execute($stmt)){
                if(mysqli_stmt_bind_result($stmt, $floorDescription)){
...
if($mode=="saveedit" or $mode=="saveadd"){
    $errorMessage="";
    if($floorDescription==""){
        $errorMessage="لابد من ادخال اسم الدور<br>";
    }
...
//*****************************************************************************************
if($mode=="saveedit"){
    $q="UPDATE floors SET floorDescription=? WHERE floorId=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"si",$floorDescription,$floorId)){
            try{
                if(mysqli_stmt_execute($stmt)){
                    $infoMessage="حفظ!...";
                }    
...
//*****************************************************************************************
if($mode=="saveadd"){
    $q="INSERT INTO floors(floorDescription) VALUES(?)";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"s",$floorDescription)){
            if(mysqli_stmt_execute($stmt)){
...
//*****************************************************************************************
if($mode == "delete"){
    $q="DELETE FROM floors WHERE floorId=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"i",$floorId)){
            if(mysqli_stmt_execute($stmt)){
...
if($mode=="add" or $mode=="edit"){
    ...
    echo "<tr><td style='width: 100px;'>اسم الدور:</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<input class='input-field' type='text' placeholder='اسم الدور' maxlength='255' name='floorDescription'";
    if(isset($floorDescription)){
        echo " value='$floorDescription'";
    } 
    echo "></div></td></tr>";  

    echo "</table>";
    if($mode=='edit'){
        echo "<input type='hidden' name='floorId' value='$floorId'>";
    }
    $newMode="save$mode";
...
if($mode == "view"){
    echo "<center>";
    echo "<h3> بيانات الدور </h3>";
    echo "<table>";
    echo "<tr><td>Description</td><td>:</td><td align='right'> $floorDescription </td></tr>";
    echo "</table>";
...
if($mode=="deleteconfirm"){
    echo "<center>";
    echo "<h3> الغاء الدور </h3>";
    echo "<div style='text-align: center;'>سيتم الغاء $floorDescription <br> هل أنت متأكد؟...</div><br>";
...
if($mode=="showList"){
    ...
    if($infoMessage!=""){
        ...
        echo "<table id='masterTable'><tr class='header'><th> الدور </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
        $q="SELECT floorId, floorDescription FROM floors ORDER BY floorId";
        $res=mysqli_query($dbc,$q);
        while($row=mysqli_fetch_assoc($res)){
            echo "<tr><td align='right'>".$row['floorDescription']."</td>

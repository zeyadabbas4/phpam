...
$prmOk=false;
$pageTitle="الدراسات اﻷساسية للبحارة ";
$pageSubTitle="المستحقات المالية";
$errorMessage="";
?>
<!DOCTYPE html>
<html dir="<?php echo $__dir; ?>">
    <head>
        <meta charset="utf-8">
        <title><?php echo $pageTitle . "-" . $pageSubTitle;?></title>
        <style>
    		* {
    ...
    <body>
	<br><h2><?php echo $pageTitle;?></h2><h3><?php echo $pageSubTitle;?></h3>
<?php
if($sesOk){
    ...
    if(checkUserMenuItem($__uid,$mnuId)){                       //check for user permission to use the screen
        ...
        echo "<br><div class='alertinfoMsgs' style='direction: ltr'>Access denied!...</div>";    
    }   
}else{
    //session not up display error!...
    echo "<br><div class='alertinfoMsgs' style='direction: ltr'>Session expired please re-login!...</div>";
}
...
if($mode=="printBenefits"){
	if($clhPeriod == 0){
		...
		if($period['Status'] != $formCourseStatusLevel +1){
			$errorMsg="الدورة لم تراجع بعد";
		}else{
			$_SESSION['CoursId']=$clsId;
			$_SESSION['clhPeriod']=$clhPeriod;
			$clsPrtBenifits++;
			$q="update Courses set CoursPrtBenifits=? where CoursId=?";
			if ($stmt = mysqli_prepare($dbc, $q)){
			  if(mysqli_stmt_bind_param($stmt, "ii", $clsPrtBenifits,$clsId)){
  ...
  if($mode=="printCourseData"){
	if($clhPeriod == 0){
		...
		if($period['Status'] != $formCourseStatusLevel +1){
			...
			if ($stmt = mysqli_prepare($dbc, $q)){
			  if(mysqli_stmt_bind_param($stmt, "ii", $clsPrtCourseData,$clsId)){
				if(!mysqli_stmt_execute($stmt)){
				  $errorMsg.= "Error saving reading [030103".$__uid.date("YmdHis")."]!...<br>";
				  appendLog($logDir,$logFile,$q." - ".mysqli_error($dbc),"030103".$__uid.date("YmdHis"));    
				}
...
//****************************************************************************************
	if($showList){
		if(isset($errorMsg)){
			echo "<div class='alertinfoMsgs'>$errorMsg</div>";
			echo "<br><br>";
		}
		if(isset($infoMsg)){
			echo "<div class='infoinfoMsgs'>$infoMsg</div>";
			echo "<br><br>";
		}
		//select batch form
		...
		if(isset($batchId)){
			$classes=array();
			$q="SELECT CoursId,CoursDescription,CoursCrsId,CoursStatus,CoursPrtBenifits,CoursPrtSupervision,CoursPrtCourseData FROM Courses WHERE CoursType=5 AND (CoursStatus >= ? OR CoursId IN (SELECT crprCourseId FROM coursPeriods WHERE crprStatus >= ?)) AND CoursYear = ? ORDER BY CoursStatus";
			if($stmt = mysqli_prepare($dbc, $q)){
				if(mysqli_stmt_bind_param($stmt, "iii", $formCourseStatusLevel,$formCourseStatusLevel,$batchId)){
					if(mysqli_stmt_execute($stmt)){
						if(mysqli_stmt_bind_result($stmt,$CoursId,$CoursDescription,$CoursCrsId,$CoursStatus,$CoursPrtBenifits,$CoursPrtSupervision,$CoursPrtCourseData)){
							while(mysqli_stmt_fetch($stmt)){
								$classes[$CoursId]['Description']=$CoursDescription;
								$classes[$CoursId]['Speciality']=$CoursCrsId;
								$classes[$CoursId]['Status']=$CoursStatus;
								$classes[$CoursId]['PrtBenifits']=$CoursPrtBenifits;
								$classes[$CoursId]['PrtSupervision']=$CoursPrtSupervision;
								$classes[$CoursId]['PrtCourseData']=$CoursPrtCourseData;
							}
						}
					}
				}		
			...
			foreach($classes as $key => $value){
				...
				$clsPrtBenifits=$value['PrtBenifits'];
				$clsPrtSupervision=$value['PrtSupervision'];
				$clsPrtCourseData=$value['PrtCourseData'];
				$printedMessage="";
				...
				$courseDataBadge="";
				$devInfo="";
				if($clsStatus > $formCourseStatusLevel){       //When course is abroved by the dean
					$BenefitsDisabled="";
					$courseDataDisabled="";
				}
				if(isset($perms['Developer'])){
				...
				if($clsPrtBenifits>0 or $clsPrtCourseData>0){
					$printedMessage="[تمت الطباعة]";
				}
				echo "<tr>";
				echo "<td>$clsDescription $printedMessage $devInfo</td>";
				echo "<td style='width: $buttonCellWidth;text-align: left;'>";
				echo "<form method='post'>";
				echo "<input type='hidden' name='clsPrtBenifits' value='$clsPrtBenifits'>";
				echo "<input type='hidden' name='clsPrtCourseData' value='$clsPrtCourseData'>";
				echo "<input type='hidden' name='clsId' value='$clsId'>";
				echo "<input type='hidden' name='batchId' value='$batchId'>";
				$periodListDisabled="";
		  		...
		  		foreach($periods as $key => $value){
					if($value['Status'] >= $formCourseStatusLevel){
						...
						if($value['Status']==$formCourseStatusLevel){
							$tag="(جديد)";
							$buttonClass="viewBtn";
						}elseif($value['Status'] == $formCourseStatusLevel + 1){
							$tag="(تمت المراجعة)";
                            $buttonClass="grpBtn";
							$BenefitsDisabled="";
							$courseDataDisabled="";
				...
				if($clsPrtBenifits>0){
					if($clsPrtBenifits>3){
						...
						$colorIndex=$clsPrtBenifits;
					}
					$BenefitsDisabled=" style='background-color:".$buttonColors[$colorIndex].";'";
					$benefitsBadge="<sup class='badge'>$clsPrtBenifits</sup>";
					$reprintRequestDisabled="";
				}
				if($clsPrtCourseData>0){
					if($clsPrtCourseData>3){
						$colorIndex=3;
					}else{
						$colorIndex=$clsPrtCourseData;
					}
					$courseDataDisabled=" style='background-color:".$buttonColors[$colorIndex].";'";
					$courseDataBadge="<sup class='badge'>$clsPrtCourseData</sup>";
					$reprintRequestDisabled="";
				}
				$viewButttonStyle="";
				...
				echo "<button type='submit' class='$buttonClass' name='mode' value='view'$viewButttonStyle>عرض</button> ";

...
if($prmOk){
	...
	$crsTypes=array('مخطط','تعاقدي','محاكي','حتميات');
//*****************************************************************************************
//sec:edit-view-deleteconfirm read a record for view or edit
//*****************************************************************************************
    if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
        $q="SELECT stepName,stepNo,stepType,CrstpDescription FROM steps INNER JOIN CourseTypes ON stepType=CrstpId WHERE stepId=?";
        if($stmt=mysqli_prepare($dbc, $q)){
            if(mysqli_stmt_bind_param($stmt, "i", $stepId)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt, $stepName,$stepNo,$stepType,$CrstpDescription)){
...
//*****************************************************************************************
	if($mode=="saveedit" or $mode=="saveadd"){
		$errorMessage="";
		if($stepName==""){
			$errorMessage="لابد من ادخال step name<br>";
		}
		if($stepNo==""){
			$errorMessage="لابد من ادخال step number<br>";
		}
		if($stepType=="0"){
			$errorMessage="لابد من اختيار step type<br>";
		}		
		if($errorMessage != ""){
			$mode=substr($mode,4);
		}
...
//*****************************************************************************************
	if($mode=="saveedit"){
		$q="UPDATE steps SET stepName=?,stepNo=?,stepType=? WHERE stepId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"siii",$stepName,$stepNo,$stepType,$stepId)){
				try{
					if(mysqli_stmt_execute($stmt)){
						$infoMessage="Saved!...";
					}	
...
//*****************************************************************************************
	if($mode=="saveadd"){
		$q="INSERT INTO steps(stepName,stepNo,stepType) VALUES(?,?,?)";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"sii",$stepName,$stepNo,$stepType)){
				if(mysqli_stmt_execute($stmt)){
					$infoMessage="Saved!...";
				}else{
...
//*****************************************************************************************
	if($mode == "delete"){
		$q="DELETE FROM steps WHERE stepId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"i",$stepId)){
				if(mysqli_stmt_execute($stmt)){
					$infoMessage="Deleted!...";
...
//*****************************************************************************************
	if($mode=="add" or $mode=="edit"){
		...
		echo "<tr><td style='width: 100px;'>Step name:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<input class='input-field' type='text' placeholder='Step name' maxlength='255' name='stepName'";
		if(isset($stepName)){
			echo " value='$stepName'";
		} 
		echo "></div></td></tr>";  

		echo "<tr><td style='width: 100px;'>Step number:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<input class='input-field' type='text' placeholder='Step number' maxlength='255' name='stepNo'";
		if(isset($stepNo)){
			echo " value='$stepNo'";
		} 
		echo "></div></td></tr>";  

		echo "<tr><td style='width: 100px;'>Step type:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<select class='input-field' name='stepType'><option value='-1'>Select Type</option>";
			...
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt,$CrstpId,$CrstpDescription)){
					while(mysqli_stmt_fetch($stmt)){
						echo "<option value='$CrstpId'";
						if(isset($stepType)){
							if($stepType==$CrstpId){
								echo " selected";
							}	
						}
						echo ">$CrstpDescription</option>";
					}
				}
			}    
            mysqli_stmt_close($stmt);
        ...
        echo "<div class='frmButtons'><button type='submit' class='savBtn' name='mode' value='$newMode'> Save </button> <button type='button' class='cnlBtn' onclick='window.location=\"$filename\";'> تراجع </button></div>";
		echo "</form>";
		echo "</center>";	
	}
...
//*****************************************************************************************
	if($mode == "view"){
		echo "<center>";
		echo "<h3> Step Data </h3>";
		echo "<table>";
		echo "<tr><td>Step name</td><td>:</td><td align='right'> $stepName </td></tr>";
		echo "<tr><td>Step number</td><td>:</td><td align='right'> $stepNo </td></tr>";
		echo "<tr><td>Step type</td><td>:</td><td align='right'> $CrstpDescription </td></tr>";
		echo "</table>";
...
//*****************************************************************************************
	if($mode=="deleteconfirm"){
		echo "<center>";
		echo "<h3> Delete Step </h3>";
		echo "<div style='text-align: center;'>سيتم الغاء $stepName <br> هل أنت متأكد؟...</div><br>";
...
//****************************************************************************************
	if($mode=="showList"){
		...
		//select Course Type
		echo "<div style='width: 220px; margin: auto;direction: ltr;'><form method='post' id='typeForm'>";
		echo "Step type filter: <select class='input-field' name='stepType' onchange='document.getElementById(\"typeForm\").submit();'>";
		$q="SELECT CrstpId,CrstpDescription FROM CourseTypes ORDER BY CrstpId";
		if($stmt=mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt,$CrstpId,$CrstpDescription)){
					while(mysqli_stmt_fetch($stmt)){
		...
		echo "<table id='masterTable'><tr class='header'><th> المؤلف </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
		$q="SELECT stepId,stepName,stepNo FROM steps WHERE stepType=? ORDER BY stepNo";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"i",$stepType)){
				if(mysqli_stmt_execute($stmt)){
					if(mysqli_stmt_bind_result($stmt, $stepId,$stepName,$stepNo)){
						while(mysqli_stmt_fetch($stmt)){
							echo "<tr>";
							echo "<td> $stepNo - $stepName </td>";

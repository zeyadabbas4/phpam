...
if($mode=="reprintRequest"){
  ...
  $q="INSERT INTO courseReprintLog(crlUserName, crlDate, crlTime, crlFlag, crlCourseId, crlCourseType, crlBulleten) VALUES (?,?,?,'CoursPrtBenifits',?,3,?)";
  if ($stmt = mysqli_prepare($dbc, $q)){
  ...
  $q="INSERT INTO courseReprintLog(crlUserName, crlDate, crlTime, crlFlag, crlCourseId, crlCourseType, crlBulleten) VALUES (?,?,?,'CoursPrtCourseData',?,3,?)";
  if ($stmt = mysqli_prepare($dbc, $q)){
    if(mysqli_stmt_bind_param($stmt, "sssii", $requestUser['usrFullName'], $requestDate,$requestTime,$CoursId,$CoursBulletin)){
      if(!mysqli_stmt_execute($stmt)){
        $errorMsg.= "Error saving reading [030103".$__uid.date("YmdHis")."]!...<br>";
...
/*********************************************************************************************************** */
    if($showList){
      if($errorMsg != ""){
        echo "<div class='error'>$errorMsg<br><br></div>";
      }
      if(isset($infoMsg)){
        echo "<div class='infoMsg'>$infoMsg</div>";
        $infoMsg="";
      }
      $q="SELECT CoursId,CoursBulletin,CrsName,CoursStatus,CoursPrtBenifits,CoursPrtCourseData FROM Courses inner join CoursesGuide on CoursCrsId=CrsId WHERE CoursType=$CoursType and CoursYear=".$user_year['Id']." and CoursStatus >=$formCourseStatusLevel order by CoursStatus,CoursBulletin";

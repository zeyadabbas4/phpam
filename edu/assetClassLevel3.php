...
if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
    $q="SELECT acl3Id, acl3Desc, acl3Level2 FROM assetClassLevel3 WHERE acl3Id=?";
    if($stmt=mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt, "s", $acl3Id)){
            if(mysqli_stmt_execute($stmt)){
                if(mysqli_stmt_bind_result($stmt, $acl3Id, $acl3Desc, $acl3Level2)){
...
if($mode=="saveedit" or $mode=="saveadd"){
    ...
    // Validate acl3Desc (it should not be empty)
    if(trim($acl3Desc) == ""){
        $errorMessage .= "لابد من ادخال اسم التكويد<br>";
    }
    
    // Validate acl3Id (it should be a valid number if it's a new record)
    if($mode == "saveadd" && (empty($acl3Id) || !is_numeric($acl3Id))){
...
//*****************************************************************************************
if($mode=="saveedit"){
    $q="UPDATE assetClassLevel3 SET acl3Desc=?, acl3Level2=? WHERE acl3Id=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"ssi",$acl3Desc,$acl3Level2,$acl3Id)){
            try{
                if(mysqli_stmt_execute($stmt)){
                    $infoMessage="حفظ!...";
                }    
...
if($mode=="saveadd"){
    // Validate input
    if(empty($acl3Desc) || empty($acl3Id) || empty($acl3Level2)) {
        $errorMessage = "لابد من إدخال اسم التكويد والكود والفئة الرئيسية.";
    }
    
    if(empty($errorMessage)){
        $q = "INSERT INTO assetClassLevel3(acl3Id, acl3Desc, acl3Level2) VALUES(?, ?, ?)";
        
        if ($stmt = mysqli_prepare($dbc, $q)) {
            // Bind acl3Id, acl3Desc, and acl3Level2
            if (mysqli_stmt_bind_param($stmt, "sss", $acl3Id, $acl3Desc, $acl3Level2)) {
                if (mysqli_stmt_execute($stmt)) {
...
//*****************************************************************************************
if($mode == "delete"){
    $q="DELETE FROM assetClassLevel3 WHERE acl3Id=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"s",$acl3Id)){
            if(mysqli_stmt_execute($stmt)){
...
if($mode=="add" or $mode=="edit"){
    ...
    // ID field
    echo "<tr><td style='width: 100px;'>  كود المستوي الثالث:</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<input class='input-field' type='text' placeholder=' ادخل الكود ' maxlength='255' name='acl3Id' value='$acl3Id'>";
    echo "</div></td></tr>";
    
    // Description field
    echo "<tr><td style='width: 100px;'>الاسم :</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<input class='input-field' type='text' placeholder='الاسم ' maxlength='255' name='acl3Desc' value='$acl3Desc'>";
    echo "</div></td></tr>";
    
    // Level1 dropdown
    echo "<tr><td style='width: 100px;'>كود المستوي التاني:</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<select class='input-field' name='acl3Level2'>";
    // Query to fetch level1 options
    $q = "SELECT acl2Id, acl2Desc FROM assetClassLevel2";
    $result = mysqli_query($dbc, $q);
    while ($row = mysqli_fetch_assoc($result)) {
        $selected = ($row['acl2Id'] == $acl3Level2) ? "selected" : "";
        echo "<option value='" . $row['acl2Id'] . "' $selected>" . $row['acl2Desc'] . "</option>";
    }
    echo "</select>";
    echo "</div></td></tr>";
    ...
    echo "</table>";
    if($mode=='edit'){
        echo "<input type='hidden' name='acl3Id' value='$acl3Id'>";
    }
    $newMode="save$mode";
...
if($mode == "view"){
    ...
    echo "<table>";
    echo "<tr><td>كود المستوي الثلث </td><td>:</td><td align='right'> $acl3Id </td></tr>";
    echo "<tr><td>الاسم </td><td>:</td><td align='right'> $acl3Desc </td></tr>";
    echo "<tr><td>كود المستوي التاني</td><td>:</td><td align='right'> $acl3Level2 </td></tr>";
    echo "</table>";
...
if($mode=="deleteconfirm"){
    ...
    echo "<div style='text-align: center;'>سيتم الغاء التكويد رقم $acl3Id - $acl3Desc <br> هل أنت متأكد؟...</div><br>";
...
if($mode=="showList"){
    ...
    if($infoMessage!=""){
        ...
        echo "<table id='masterTable'><tr class='header'><th> كودالمستوي الثالث </th><th>الاسم</th><th>كود المستوي التاني </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
        
        $q="SELECT acl3Id, acl3Desc, acl3Level2 FROM assetClassLevel3 ORDER BY acl3Id";
        $res=mysqli_query($dbc,$q);
        while($row=mysqli_fetch_assoc($res)){
            echo "<tr>
                    <td align='right'>" . $row['acl3Id'] . "</td>
                    <td align='right'>" . $row['acl3Desc'] . "</td>
                    <td align='right'>" . $row['acl3Level2'] . "</td>
                    <td align='center' style='width: $buttonCellWidth'>
                    <form method='post'>
                    <input type='hidden' name='acl3Id' value='" . $row['acl3Id'] . "'>

...
if(isset($_SESSION['__uid'])){
        ...
        if($logged){
                ...
                if($mode == 'SaveNewUser'){
                        ...
                        echo "</div>";
                        if(!$err){
                                $q="insert into UserYears(UYuid,UYyid,UYsid) values(?,?,'F')";
                ...
                if($mode == 'SaveEditUser')            
                {
                        ...
                        echo "</div>";
                        if(!$err){
                                $q="update UserYears set UYuid=?,UYyid=? where UYuid=? and UYyid=? and UYsid='F'";
                                if($stmt = mysqli_prepare($dbc, $q)){
                ...
                {
                        $q="delete from UserYears where UYuid=? and UYyid=? and UYsid='F'";
                        if($stmt = mysqli_prepare($dbc, $q)){
                                mysqli_stmt_bind_param($stmt, "ii", $UYuid,$UYyid);
                                if(!mysqli_stmt_execute($stmt)){
                                        $errorMessage.= "Error saving data!...<br>";
                ...
                if($mode == 'NewUser' || $mode == 'EditUser'){      //Add/Edit forms
                        ...
                        include('connect.php');
                        $q="select * from Years where YearType='F' order by YearId";
                ...
                if($showtable){
                        ...
                        $rowNo=0;
                        $q="select * from UserYears inner join Years on UYyid=YearId where UYsid='F'";
                                $r=mysqli_query($dbc,$q);
                                if($r){
                                        while($row=mysqli_fetch_array($r)){
                                                $UYuid=$row['UYuid'];

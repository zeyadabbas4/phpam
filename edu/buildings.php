...
if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
	$q="SELECT bldDescription FROM buildings WHERE bldId=?";
	if($stmt=mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt, "i", $bldId)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt, $bldDescription)){
...
if($mode=="saveedit" or $mode=="saveadd"){
	$errorMessage="";
	if($bldDescription==""){
		$errorMessage="لابد من ادخال اسم المبنى<br>";
	}
...
//*****************************************************************************************
//sec:sqveedit save edit data
//*****************************************************************************************
if($mode=="saveedit"){
	$q="UPDATE buildings SET bldDescription=? WHERE bldId=?";
	if ($stmt = mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt,"si",$bldDescription,$bldId)){
			try{
				if(mysqli_stmt_execute($stmt)){
					$infoMessage="حفظ!...";
				}	
...
//*****************************************************************************************
if($mode=="saveadd"){
	$q="INSERT INTO buildings(bldDescription) VALUES(?)";
	if ($stmt = mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt,"s",$bldDescription)){
			if(mysqli_stmt_execute($stmt)){
...
//*****************************************************************************************
if($mode == "delete"){
	$q="DELETE FROM buildings WHERE bldId=?";
	if ($stmt = mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt,"i",$bldId)){
			if(mysqli_stmt_execute($stmt)){
...
if($mode=="add" or $mode=="edit"){
	...
	if($mode=="add"){
		$formTitle="اضف مبنى جديد";
	}else{
		$formTitle="تعديل المبنى";
	}
	...
	echo "<tr><td style='width: 100px;'>اسم المبني:</td><td style='width:650px;'>";
	echo "<div class='input-container'>";
	echo "<input class='input-field' type='text' placeholder='اسم المبني' maxlength='255' name='bldDescription'";
	if(isset($bldDescription)){
		echo " value='$bldDescription'";
	} 
	echo "></div></td></tr>";  

	echo "</table>";
	if($mode=='edit'){
		echo "<input type='hidden' name='bldId' value='$bldId'>";
	}
	$newMode="save$mode";
...
if($mode == "view"){
	echo "<center>";
	echo "<h3> بيانات المبنى </h3>";
	echo "<table>";
	echo "<tr><td>Description</td><td>:</td><td align='right'> $bldDescription </td></tr>";
	echo "</table>";
...
if($mode=="deleteconfirm"){
	echo "<center>";
	echo "<h3> الغاء مبنى </h3>";
	echo "<div style='text-align: center;'>سيتم الغاء $bldDescription <br> هل أنت متأكد؟...</div><br>";
...
if($mode=="showList"){
	...
	if($infoMessage!=""){
		...
		echo "<table id='masterTable'><tr class='header'><th> المبنى </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
		$q="SELECT bldId, bldDescription FROM buildings ORDER BY bldId";
		$res=mysqli_query($dbc,$q);
		while($row=mysqli_fetch_assoc($res)){
			echo "<tr><td align='right'>".$row['bldDescription']."</td>
					<td align='center' style='width: $buttonCellWidth'>
					<form method='post'>
					<input type='hidden' name='bldId' value='".$row['bldId']."'>

...
if(isset($_SESSION['__uid'])){
    ...
    if($crsId != -1){
        $q="SELECT CoursId, CoursStatus, CrsCode, CoursCrsId, CoursGenRept, CrsName, CoursFromAct, CoursToAct, CoursBulletin, CoursSupervisorExt, dist_name, CrsDescription, CrsQualification, CrsPrerequisits, CrsDepartments, trntpDescription, cmpName, s1.staffname staffnameint, COALESCE(s2.staffname,'-') staffnameext,CoursLetterName FROM (((((Courses INNER JOIN CoursesGuide ON CoursCrsId = CrsId) inner join districts on dist_id = CoursArea) inner join trainingTypes on trntpId=CrsTrnType) inner join companies on CoursLocation=cmpId) inner join staff s1 on CoursSupervisorInt=s1.staffid) left join staff s2 on CoursSupervisorExt=s2.staffid WHERE CoursId=?";
        if ($stmt = mysqli_prepare($dbc, $q)) {
            if(mysqli_stmt_bind_param($stmt, "i", $crsId)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt, $CoursId, $CoursStatus, $CrsCode, $CoursCrsId, $CoursGenRept, $CrsName, $CoursFromAct, $CoursToAct, $CoursBulletin, $CoursSupervisorExt, $dist_name, $CrsDescription, $CrsQualification, $CrsPrerequisits, $CrsDepartments, $trntpDescription, $cmpName, $staffnameint, $staffnameext,$CoursLetterName)){
        ...
        if($readOne){
            $contacts=array();
            $q="SELECT cmpId,cmpName,conttitle,contname,contjob FROM companies INNER JOIN contacts ON cmpId=contcompany ORDER BY cmpId";
            if ($stmt = mysqli_prepare($dbc, $q)) {
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt, $cmpId,$cmpName,$conttitle,$contname,$contjob)){
            ...
            $totPlanned=0;
            $conts=array();
            $q="SELECT crstrntrainee,crstrncompany,crstrncourse FROM coursetrainees WHERE crstrncourse=? ORDER BY crstrncompany";
            if ($stmt = mysqli_prepare($dbc, $q)) {
                if(mysqli_stmt_bind_param($stmt, "i", $crsId)){
                    if(mysqli_stmt_execute($stmt)){
                        if(mysqli_stmt_bind_result($stmt, $crstrntrainee,$crstrncompany,$crstrncourse)){
                            while(mysqli_stmt_fetch($stmt)){
                                if($lastComp != $crstrncompany){
                                    $conts[$crstrncompany]['Name']=$contacts[$crstrncompany]['Name'];
                                    $conts[$crstrncompany]['Count']=1;
                                    $conts[$crstrncompany]['contact']= $contacts[$crstrncompany]['contact'];
                                    $conts[$crstrncompany]['contjob']=$contacts[$crstrncompany]['contjob'];    
                                    $lastComp=$crstrncompany;
                                }else{
                                    $conts[$crstrncompany]['Count']++;
                                }
                                $totPlanned++;
                            }
                        }
...
*/
    if($assignTo == -1 or !isset($assignTo)){
    ...
    echo "
        <body>
            <div class='buttons'>
                $errorMessage
                <h2>طباعة خطاب النتيجة لدورة $CrsName</h2>";
                if(!isset($pe)){
                    echo "<form method='post'><table width='400px' align='center'>";
                    //echo "<tr><td>رقم القيد</td><td>:</td><td align='right'><input type='text' name='letterRegNo' value='$letterRegNo'></td></tr>
                    //echo "<tr><td>التاريخ</td><td>:</td><td align='right'><input type='date' name='letterDate' value='$letterDate'></td></tr>";
                    echo "<tr><td>التوجيه</td><td>:</td><td align='right'><select name='assignTo'><option value='-1'>---</option>";
                ...
                if(!$validationError && $submitted == '1'){
                    ...
                    echo "<input type='hidden' name='printEnabled' value='$printEnabled'>";
                    echo "<button type='button' class='cnlBtn' onclick='window.close();'>إغلاق</button>";
                }
                echo "
                <hr><br>
    ...
    if(!$validationError && $submitted == '1'){
        ...
        <div class='main-doc'>
            ...
            <br>
            <hr>";
//            echo "<p><span class='line-title'>رقم القيد: </span> $letterRegNo</p>";
//            echo "<p><span class='line-title'>التاريخ: </span> $letterDate </p>";
            echo "<p><span class='line-title'>رقم القيد: </span> &nbsp; </p>";
            echo "<p><span class='line-title'>التاريخ: </span> &nbsp; </p>";
            echo "<p></p>
            <p>".$contacts[$assignTo]['contact']."</p>
            <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$contacts[$assignTo]['contjob']."</p>
            <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$contacts[$assignTo]['Name']."</p>
            <p>تحية طيبة وبعد،،،</p>
            <p>مرسل لسيادتكم طيه نتيجة الدورة رقم منشور ( $CoursBulletin ) للعام التدريبي ".$user_year['Desc']." بعنوان:-</p>
            <p class='crs-title'>$CrsName</p>
            <p>رقم ($CoursGenRept) كود ($CrsCode) والتي عقدت بمنطقة $dist_name لعدد ($totPlanned) مشارك خلال الفترة من $CoursFromAct الى $CoursToAct وذلك تحت اشراف معهد تدريب الموانئ.</p>

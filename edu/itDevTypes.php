...
<html dir="<?php echo $gdir; ?>">
    ...
    <body>
	<br><h2 style='text-align: center;color: RoyalBlue;'>أنواع اﻷجهزة</h2>
<?php
if($sesOk){
...
//*****************************************************************************************
    if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
        $q="SELECT TypeDesc FROM DevTypes WHERE TypeId=?";
        if($stmt=mysqli_prepare($dbc, $q)){
            if(mysqli_stmt_bind_param($stmt, "i", $TypeId)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt, $TypeDesc)){
...
//*****************************************************************************************
	if($mode=="saveedit" or $mode=="saveadd"){
		$errorMessage="";
		if($TypeDesc==""){
			$errorMessage="لابد من ادخال اسم النوغ<br>";
		}
...
//*****************************************************************************************
	if($mode=="saveedit"){
		$q="UPDATE DevTypes SET TypeDesc=? WHERE TypeId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"si",$TypeDesc,$TypeId)){
				if(mysqli_stmt_execute($stmt)){
					$message="تم الحفظ!...";
				}    
			}
...
//*****************************************************************************************
	if($mode=="saveadd"){
		$q="INSERT INTO DevTypes(TypeDesc) VALUES(?)";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"s",$TypeDesc)){
				if(mysqli_stmt_execute($stmt)){
					$message="تم الحفظ!...";
				}    
			}
...
//*****************************************************************************************
	if($mode == "delete"){
		$q="DELETE FROM DevTypes WHERE TypeId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"i",$TypeId)){
				if(mysqli_stmt_execute($stmt)){
					$message="تم الالغاء!...";
...
//*****************************************************************************************
	if($mode=="add" or $mode=="edit"){
		...
		echo "<tr><td style='width: 100px;'>اسم النوغ:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<input class='input-field' type='text' placeholder='اسم النوغ' maxlength='255' name='TypeDesc'";
		if(isset($TypeDesc)){
			echo " value='$TypeDesc'";
		} 
		echo "></div></td></tr>";  
...
//*****************************************************************************************
	if($mode == "view"){
		$showList=false;
		echo "<center>";
		echo "<h2 style='text-align: center;'> بيانات النوغ </h2>";
		echo "<table>";
		echo "<tr><td>اسم النوغ</td><td>:</td><td align='right'> $TypeDesc </td></tr>";
		echo "</table>";
...
//*****************************************************************************************
	if($mode=="deleteconfirm"){
		$showList=false;
		echo "<center>";
		echo "<h2 style='text-align: center;'> الغاء نوغ </h2>";
		echo "<div style='text-align: center;'>سيتم الغاء $TypeDesc <br> هل أنت متأكد؟...</div><br>";
...
//****************************************************************************************
	if($showList){
		...
		echo "<table id='masterTable'><tr class='header'><th> النوع </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
		$q="SELECT TypeId,TypeDesc FROM DevTypes ORDER BY TypeDesc";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt, $TypeId,$TypeDesc)){
					while(mysqli_stmt_fetch($stmt)){
						echo "<tr>";
						echo "<td> $TypeId - $TypeDesc </td>";

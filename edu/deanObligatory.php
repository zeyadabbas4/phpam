...
if(isset($_SESSION['__uid'])){
  ...
  if($logged){
    ...
    $pageTitle="الدورات الحتمية لعام ".$user_year['Desc'];
    $pageSubTitle="مكتب العميد - إعتماد نتيجة الدورات الحتمية";
...
/*********************************************************************************************************** */
    if($showList){
      ...
      $q="SELECT CoursId,CoursBulletin,CrsName,CoursStatus FROM Courses inner join CoursesGuide on CoursCrsId=CrsId WHERE CoursType=4 and CoursYear=".$user_year['Id']." and CoursStatus >=$formCourseStatusLevel order by CoursStatus,CoursBulletin";

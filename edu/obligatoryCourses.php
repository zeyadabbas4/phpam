...
//***********************************************************************************************
    $supers=array();
    $q="SELECT `staffid`,`staffname` FROM `staff` WHERE `staffislec`=1 AND `staffdeleted`=0 AND (`staffcontract`=1 OR `staffcontract`=4) ORDER BY `staffname`";
    $r=mysqli_query($dbc,$q);
    if($r){
      while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
        foreach($row as $key => $value){
...
//***********************************************************************************************
    $prgs=array();
    $q="select CrsId,CrsName from CoursesGuide where CrsProgram=10;";
    $r=mysqli_query($dbc,$q);
    if($r){
      while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
        foreach($row as $key => $value){
...
//***********************************************************************************************
    ...
    echo "<title>$pageTitle - $trnYear - الدورات الحتمية</title>";
    echo "<style>";
    ...
    echo "</style>";
	  echo "</head>";
	  echo "<body>";
    echo "<br><h2 style='text-align: center;color: RoyalBlue;'>$pageTitle للعام $trnYear  - الدورات الحتمية</h2>";
...
//*****************************************************************************************
    if($mode=="saveadd" or $mode=="saveedit"){
      ...
      if($CoursFromPln < $yearStart){
        $errorMessage.="تاريخ الدورة قبل بداية العام التدريبي<br>";
        $mode=substr($mode,4);
      }
      if($CoursFromPln > $yearEnd){
        $errorMessage.="تاريخ الدورة بعد نهاية العام التدريبي<br>";
        $mode=substr($mode,4);
      }
      if($CoursLocation == '-1'){
        $errorMessage="لابد من تحديد مكان الانعقاد";
        $mode=substr($mode,4);
      }
      if($CoursSupervisorInt == '-1'){
        $errorMessage="لابد من تحديد الإشراف";
        $mode=substr($mode,4);
      }
      if($mode=="saveadd"){
...
//*****************************************************************************************
    if($mode=="saveadd"){
      $q="insert into Courses(CoursCrsId,CoursType,CoursBulletin,CoursArea,CoursSection,CoursYear,CoursSupervisorInt,CoursFromPln,CoursLocation) values($CoursCrsId,4,$CoursBulletin,$CoursArea,13,$YearId,$CoursSupervisorInt,'$CoursFromPln',$CoursLocation)";
      $r=mysqli_query($dbc,$q);
      if($r){
...
//*****************************************************************************************
    if($mode=="saveedit"){
      //save edited value
      if(!$validationError){
        $q="UPDATE Courses SET CoursCrsId='$CoursCrsId', CoursBulletin=$CoursBulletin, CoursArea=$CoursArea, CoursFromPln='$CoursFromPln', CoursLocation=$CoursLocation, CoursSupervisorInt=$CoursSupervisorInt WHERE CoursId=$CoursId";
        $r=mysqli_query($dbc,$q);
        if($r){
          echo "<div style='color: green;text-align: center;'>تم الحفظ بنجاح</div>";
        }
...
if(($mode=="edit" or $mode=="view" or $mode=="deleteConfirm" or $mode=='saveedit') and $errorMessage == ""){
  $q="select CoursId,CoursCrsId,CrsName,CoursBulletin,CoursStatus,CoursArea,CoursFromPln,CoursLocation,CoursSupervisorInt from CoursesGuide inner join Courses on CoursCrsId=CrsId where CoursId=$CoursId";
  $r=mysqli_query($dbc,$q);
  if($r){
    if($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
      foreach($row as $key => $value){
...
//*****************************************************************************************
    if($mode=="add" or $mode=="edit"){
      ...
      if(isset($CoursSupervisorInt)){
        $CoursSupervisor=$CoursSupervisorInt;
      }else{
        $CoursSupervisor=$intSuper;
      }

      echo "<tr><td style='width: 225px;'>الاشراف:</td><td colspan='2'>";
      echo "<div class='input-container'>";
      echo "<select class='input-field' name='CoursSupervisorInt'><option value='-1'>---</option>";
      foreach($supers as $key => $value){
      echo "<option value='$key'";
        if($CoursSupervisor  == $key){
          echo " selected";
        }
        echo ">$value</option>";  
      }
...
//*****************************************************************************************
    if($mode=="view"){
      ...
      echo "<tr><td style='width: 225px;'>الإشراف:</td><td colspan='2'><div class='input-container'>".$supers[$CoursSupervisorInt]."</div></td></tr>";
      echo "</table>";
      ...
      if($r){
        ...
        while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
          ...
          if(isset($userPerms['Developer'])){
            $devInfo="Id=$CoursId Status=$CoursStatus";
          }
          ...
          if($CoursStatus == $formCourseStatusLevel){
           ...
           //   echo "<button type='submit' class='regBtn' name='mode' value='registeration'>   تسجيل مؤقت</button> ";

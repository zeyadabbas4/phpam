...
if(isset($_SESSION['__uid'])){
  ...
  if($logged){
    ...
    if($mode==5 or $mode==6){
      ...
      if($CrsPrerequisits==""){
        $CrsPrerequisits="لا يوجد";
      }
      if($CrsQualification==""){
        $errorMessage.="لا بد من ادخال المؤهلات<br>";
      }
      if($CrsDepartments==""){
        $errorMessage.="لا بد من ادخال اﻹدارات<br>";
      }
      if($CrsTHours==""){
        $CrsTHours="0";
        if($CrsPHours==""){
      ...
      //verify detail
      if(isset($tpcdesc)){
        $totCrsHours=$CrsTHours+$CrsPHours;
        $totTpHrs=0;
        foreach($tpcdesc as $key => $value){
          ...
          $totTpHrs+=$tpcphrs[$key];
          $totTpHrs+=$tpcthrs[$key];
        }
        if($totTpHrs != $totCrsHours){
          $errorMessage.="مجموع الساعات غير مطابق<br>";
        }
      }
    }
    ...
    if($mode==5){
      ...
      if($errorMessage == ""){
        $newid=-1;
        //save master
        $q="INSERT INTO CoursesGuide(CrsProgram, CrsCode, CrsName, CrsDescription, CrsQualification, CrsPrerequisits, CrsTHours, CrsPHours,CrsTrnType,CrsDepartments) VALUES (?,?,?,?,?,?,?,?,?,?);";
        if($stmt = mysqli_prepare($dbc, $q)){
          mysqli_stmt_bind_param($stmt, "isssssiiis", $CrsProgram,$CrsCode,$CrsName,$CrsDescription,$CrsQualification,$CrsPrerequisits,$CrsTHours,$CrsPHours,$CrsTrnType,$CrsDepartments);
    ...
    if($mode==1){
      ...
      $q='select * from Programs order by PrgName';
      dropdownlist($dbc,$q,'PrgId','PrgName','CrsProgram',false,$CrsProgram,false,0,'100%');
      echo "</div></td></tr>";
      ...
      echo ">";
      echo "</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>الدورات :</td><td colspan='2'>";
      echo "<div class='input-container'>";
      echo "<input class='input-field' type='text' placeholder='الدورات' name='CrsPrerequisits'";
      if(isset($CrsPrerequisits))
        echo " value='$CrsPrerequisits'";
      echo ">";
      echo "</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>اﻹدارات :</td><td colspan='2'>";
      echo "<div class='input-container'>";
      echo "<input class='input-field' type='text' placeholder='اﻹدارات' name='CrsDepartments'";
      if(isset($CrsDepartments))
        echo " value='$CrsDepartments'";
      echo ">";
      echo "</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>نوع التدريب :</td><td colspan='2'>";
      echo "<div class='input-container'>";
      if(!isset($CrsTrnType)){
        $CrsTrnType="";
      }
      $q='select * from trainingTypes';
      dropdownlist($dbc,$q,'trntpId','trntpDescription','CrsTrnType',false,$CrsTrnType,false,0,'100%');
      echo "</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>الساعات النظرية :</td><td colspan='2'>";
      echo "<div class='input-container'>";
      echo "<input class='input-field' type='number' min='0' max='500' placeholder='الساعات النظرية' name='CrsTHours'";
      if(isset($CrsTHours))
      ...
      echo "<div class='input-container'>";
      echo "<input class='input-field' type='number' min='0' max='500' placeholder='الساعات العملية' name='CrsPHours'";
      if(isset($CrsPHours))
    ...
    if($mode==6){
      //save edited value
      if($errorMessage==""){
        $q="UPDATE CoursesGuide SET CrsProgram=?,CrsCode=?,CrsName=?,CrsDescription=?,CrsQualification=?,CrsPrerequisits=?,CrsTHours=?,CrsPHours=?,CrsTrnType=?,CrsDepartments=? WHERE CrsId=?";
        if ($stmt = mysqli_prepare($dbc, $q)){
          mysqli_stmt_bind_param($stmt, "isssssiiisi", $CrsProgram,$CrsCode,$CrsName,$CrsDescription,$CrsQualification,$CrsPrerequisits,$CrsTHours,$CrsPHours,$CrsTrnType,$CrsDepartments,$CrsId);
          if(!mysqli_stmt_execute($stmt)){
            $errorMessage.= "Error saving data! ...<br>";
            $errorMessage.= mysqli_error($dbc)."<br>";
          }
    ...
    if($mode==2){
      $showList=false;
      if($errorMessage ==""){
        $q="SELECT CrsProgram,CrsCode,CrsName,CrsDescription,CrsQualification,CrsPrerequisits,CrsTHours,CrsPHours,CrsTrnType,CrsDepartments FROM CoursesGuide WHERE CrsId=?";
        if ($stmt = mysqli_prepare($dbc, $q)) {
          mysqli_stmt_bind_param($stmt, "i", $CrsId);
          mysqli_stmt_execute($stmt);
          mysqli_stmt_bind_result($stmt, $CrsProgram,$CrsCode,$CrsName,$CrsDescription,$CrsQualification,$CrsPrerequisits,$CrsTHours,$CrsPHours,$CrsTrnType,$CrsDepartments);
      ...
      dropdownlist($dbc,'select * from Programs order by PrgName','PrgId','PrgName','CrsProgram',false,$CrsProgram,false,0,'100%');
      echo "</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>أهداف الدورة :</td><td colspan='2'>";
      echo "<div class='input-container'>";
      echo "<input class='input-field' type='text' placeholder='أهداف الدورة' name='CrsDescription'";
      ...
      dropdownlist($dbc,'select * from trainingTypes','trntpId','trntpDescription','CrsTrnType',false,$CrsTrnType,false,0,'100%');
      echo "</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>الساعات النظرية :</td><td colspan='2'>";
      echo "<div class='input-container'>";
    ...
    if($mode==3){
      //View form
      $showList=false;
      $q="SELECT CrsCode,CrsName,CrsDescription,CrsQualification,CrsPrerequisits,CrsTHours,CrsPHours,PrgName,CrsDepartments,CrsTrnType,trntpDescription FROM (CoursesGuide INNER JOIN Programs ON CrsProgram = PrgId) inner join trainingTypes on trntpId=CrsTrnType WHERE CrsId=?";
      if ($stmt = mysqli_prepare($dbc, $q)) {
        mysqli_stmt_bind_param($stmt, "i", $CrsId);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $CrsCode,$CrsName,$CrsDescription,$CrsQualification,$CrsPrerequisits,$CrsTHours,$CrsPHours,$PrgName,$CrsDepartments,$CrsTrnType,$trntpDescription);
        mysqli_stmt_fetch($stmt);
        mysqli_stmt_close($stmt);
      }
      echo "<form method='post' style='max-width:$formWidth;margin:auto;direction:$__dir;'>";
      ...
      echo "<table width='100%'>";
      echo "<tr><td style='width: $labelWidth;'>رقم الدورة :</td><td>";
      echo "<div class='input-container'>$CrsId</div></td></tr>";
      ...
      echo "<tr><td style='width: $labelWidth;'>الدورات :</td><td>";
      echo "<div class='input-container'>$CrsPrerequisits</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>اﻹدارات :</td><td>";
      echo "<div class='input-container'>$CrsDepartments</div></td></tr>";
      echo "<tr><td style='width: $labelWidth;'>نوع التدريب :</td><td>";
      echo "<div class='input-container'>$trntpDescription</div></td></tr>";
      ...
      if ($stmt = mysqli_prepare($dbc, $q)) {
        ...
        while(mysqli_stmt_fetch($stmt)){
          ...
          echo "<tr><td style='color:black;background-color:$background;text-align:right;padding: 10px 10px;'>$tpcdesc</td>";
    ...
    if($showList){
      ...
      if($r){
        ...
        while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
          ...
          echo "<button type='submit' class='delBtn' name='mode' value='4' disabled>الغاء</button> ";
          echo "</form></td></tr>";
        }
        echo "</table>";

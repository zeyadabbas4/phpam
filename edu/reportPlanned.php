...
if(isset($mode)){
    ...
    if($errorMessage==""){
        ...
        $i=0;
        $totTrn=0;
        $q="SELECT CoursId,CoursBulletin,CrsName,CoursFromAct,CoursToAct,COUNT(crstrnid) AS traineeCount,CoursType,dist_name FROM Courses INNER JOIN CoursesGuide ON CoursCrsId=CrsId INNER JOIN districts ON CoursArea=dist_id LEFT JOIN coursetrainees ON CoursId=crstrncourse GROUP BY crstrncourse HAVING CoursFromAct>=? AND CoursFromAct<=? AND CoursType=1 ORDER BY CoursFromAct;";
        if ($stmt = mysqli_prepare($dbc, $q)){
            if(mysqli_stmt_bind_param($stmt,"ss",$datefrom,$dateto)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt,$CoursId,$CoursBulletin,$CrsName,$CoursFromAct,$CoursToAct,$traineeCount,$CoursType,$dist_name)){
        ...
        echo "<tr><td colspan='6' >إجمالي</td><td>$totTrn</td></tr>";
        echo "
            </table>";
        echo "</div>";
    }

...
if(isset($mode)){
    ...
    if($errorMessage==""){
        ...
        $q="SELECT CoursId,CoursBulletin,CrsName,CoursFromAct,CoursToAct,CoursFromPln,CouursToPln,dist_name,dist_id,PrgName,PrgId,crscmpPlanned,cmpName,cmpId,CrsTHours+CrsPHours AS CrsHours FROM Courses INNER JOIN CoursesGuide ON CoursCrsId=CrsId INNER JOIN districts ON CoursArea=dist_id INNER JOIN Programs ON CrsProgram=PrgId INNER JOIN courseCompanies ON CoursId=crscmpCourse INNER JOIN companies ON crscmpCompany=cmpId WHERE CoursType=1 AND CoursFromPln>? AND CouursToPln<? $compFilter $distFilter $progFilter ORDER BY dist_name,cmpName,PrgName,CoursFromPln";
        if ($stmt = mysqli_prepare($dbc, $q)){
            if(mysqli_stmt_bind_param($stmt,$types,...$parameters)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt,$CoursId,$CoursBulletin,$CrsName,$CoursFromAct,$CoursToAct,$CoursFromPln,$CouursToPln,$dist_name,$dist_id,$PrgName,$PrgId,$crscmpPlanned,$cmpName,$cmpId,$CrsHours)){
                            ...
                            $result[$i]['crscmpPlanned']=$crscmpPlanned;
                            $result[$i]['cmpName']=$cmpName;
                            $result[$i]['cmpId']=$cmpId;
        ...
        $q="SELECT COUNT(crstrntrainee) AS trnCountAct FROM coursetrainees WHERE crstrncompany=? AND crstrncourse=?";
        if ($stmt = mysqli_prepare($dbc, $q)){
            foreach($result as $key => $value){
                $cmpId=$value['cmpId'];
                $CoursId=$value['CoursId'];
                $crscmpPlanned=$value['crscmpPlanned'];

                if(mysqli_stmt_bind_param($stmt,"ii",$cmpId,$CoursId)){
                    if(mysqli_stmt_execute($stmt)){
        ...
        foreach($result as $key => $value){
            ...
            if($lastCompany != $value['cmpName']){
                if(!$firstCompany){
                    ...
                    echo "<tr><td colspan='6' $style>إجمالي $lastCompany</td><td $style>$cmpTotPlaned</td><td $style>$cmpTotActual</td><td $style>$cmpTotOver</td><td $style>$cmpTotUnder</td><td $style>$cmpTotHours</td></tr>";
                    $cmpTotPlaned=0;
                    $cmpTotActual=0;
                    $cmpTotOver=0;
                    $cmpTotUnder=0;
    ...
    <h2>معهد تدريب الموانئ</h2>
    <h2>إدارة التعليم والتدريب</h2>
    <h3>تقرير الدورات المخططة بالساعات</h3>
    ...
    echo "المنطقة: ";
    echo "<select name='dist' id='dist' onchange='loadCompanies(\"dist\",\"comp\");'><option value='-1'>حدد المنطقة</option>";
    ...
    echo "&nbsp;&nbsp;&nbsp;";
    echo "الشركة: ";
    echo "<select name='comp' id='comp'><option value='-1'>حدد الشركة</option>";
    echo "</select> ";
    echo "&nbsp;&nbsp;&nbsp;";
...
?>
<script>
function loadCompanies(district,companies){
    dist=document.getElementById(district);
    comp=document.getElementById(companies);
    dist_id=dist.value;
    url="fetchCompaniesByDistrict.php?distId="+dist_id;
    fetch(url)
        .then((response) => {
            if (!response.ok) {
        ...
        .then((json) => loadcmp(json))
        .catch((err) => console.error('Fetch problem: ${err.message}'));
    }
    function loadcmp(cmpData){
        var compSelect=document.getElementById("comp");
        var selectLen=compSelect.length;
        for(var i=1;i<=selectLen;i++){
            compSelect.remove(0);
        }
        var option = document.createElement("option");
            option.text = "حدد الشركة";
            option.value = "-1"
            compSelect.add(option);
        var compCount=cmpData.length;
        for(i=0;i<compCount;i++){
            var option = document.createElement("option");
            option.text = cmpData[i].name;
            option.value = cmpData[i].id;
            compSelect.add(option);
        }
    }
</script>

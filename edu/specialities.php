...
<html dir="<?php echo $gdir; ?>">
    ...
    <body>
	<br><h2 style='text-align: center;color: RoyalBlue;'>بيانات أقسام الدراسات اﻷساسية للبحارة</h2>
<?php
if($sesOk){
...
//*****************************************************************************************
    if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
        $q="SELECT spcDescription FROM specialities WHERE spcId=?";
        if($stmt=mysqli_prepare($dbc, $q)){
            if(mysqli_stmt_bind_param($stmt, "i", $spcId)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt, $spcDescription)){
...
//*****************************************************************************************
	if($mode=="saveedit" or $mode=="saveadd"){
		$errorMessage="";
		if($spcDescription==""){
			$errorMessage="لابد من ادخال اسم التخصص<br>";
		}
...
//*****************************************************************************************
	if($mode=="saveedit"){
		$q="UPDATE specialities SET spcDescription=? WHERE spcId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"si",$spcDescription,$spcId)){
				if(mysqli_stmt_execute($stmt)){
					$message="تم الحفظ!...";
				}    
			}
...
//*****************************************************************************************
	if($mode=="saveadd"){
		$q="INSERT INTO specialities(spcDescription) VALUES(?)";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"s",$spcDescription)){
				if(mysqli_stmt_execute($stmt)){
					$message="تم الحفظ!...";
				}    
			}
...
//*****************************************************************************************
	if($mode == "delete"){
		$q="DELETE FROM specialities WHERE spcId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"i",$spcId)){
				if(mysqli_stmt_execute($stmt)){
					$message="تم الالغاء!...";
...
//*****************************************************************************************
	if($mode=="add" or $mode=="edit"){
		...
		echo "<tr><td style='width: 100px;'>اسم التخصص:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<input class='input-field' type='text' placeholder='اسم التخصص' maxlength='255' name='spcDescription'";
		if(isset($spcDescription)){
			echo " value='$spcDescription'";
		} 
		echo "></div></td></tr>";  

		echo "</table>";
		if($mode=='edit'){
			echo "<input type='hidden' name='spcId' value='$spcId'>";
		}
		$newMode="save$mode";
...
//*****************************************************************************************
	if($mode == "view"){
		$showList=false;
		echo "<center>";
		echo "<h2 style='text-align: center;'> بيانات التخصص </h2>";
		echo "<table>";
		echo "<tr><td>اسم التخصص</td><td>:</td><td align='right'> $spcDescription </td></tr>";
		echo "</table>";
...
//*****************************************************************************************
	if($mode=="deleteconfirm"){
		$showList=false;
		echo "<center>";
		echo "<h2 style='text-align: center;'> الغاء مؤلف </h2>";
		echo "<div style='text-align: center;'>سيتم الغاء $spcDescription <br> هل أنت متأكد؟...</div><br>";
...
//****************************************************************************************
	if($showList){
		...
		echo "<table id='masterTable'><tr class='header'><th> التخصص </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
		$q="SELECT spcId,spcDescription FROM specialities ORDER BY spcDescription";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt, $spcId,$spcDescription)){
					while(mysqli_stmt_fetch($stmt)){
						echo "<tr>";
						echo "<td> $spcId - $spcDescription </td>";

...
if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
    $q="SELECT asflDescription FROM assetFlag WHERE asflId=?";
    if($stmt=mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt, "i", $asflId)){
            if(mysqli_stmt_execute($stmt)){
                if(mysqli_stmt_bind_result($stmt, $asflDescription)){
...
if($mode=="saveedit" or $mode=="saveadd"){
    $errorMessage="";
    if($asflDescription==""){
        $errorMessage="لابد من ادخال اسم الموقف<br>";
    }
...
//*****************************************************************************************
if($mode=="saveedit"){
    $q="UPDATE assetFlag SET asflDescription=? WHERE asflId=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"si",$asflDescription,$asflId)){
            try{
                if(mysqli_stmt_execute($stmt)){
                    $infoMessage="حفظ!...";
                }    
...
//*****************************************************************************************
if($mode=="saveadd"){
    $q="INSERT INTO assetFlag(asflDescription) VALUES(?)";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"s",$asflDescription)){
            if(mysqli_stmt_execute($stmt)){
...
//*****************************************************************************************
if($mode == "delete"){
    $q="DELETE FROM assetFlag WHERE asflId=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"i",$asflId)){
            if(mysqli_stmt_execute($stmt)){
...
if($mode=="add" or $mode=="edit"){
    ...
    echo "<tr><td style='width: 100px;'>اسم الموقف:</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<input class='input-field' type='text' placeholder='اسم الموقف' maxlength='255' name='asflDescription'";
    if(isset($asflDescription)){
        echo " value='$asflDescription'";
    } 
    echo "></div></td></tr>";  

    echo "</table>";
    if($mode=='edit'){
        echo "<input type='hidden' name='asflId' value='$asflId'>";
    }
    $newMode="save$mode";
...
if($mode == "view"){
    echo "<center>";
    echo "<h3> بيانات الموقف </h3>";
    echo "<table>";
    echo "<tr><td>Description</td><td>:</td><td align='right'> $asflDescription </td></tr>";
    echo "</table>";
...
if($mode=="deleteconfirm"){
    echo "<center>";
    echo "<h3> الغاء الموقف </h3>";
    echo "<div style='text-align: center;'>سيتم الغاء $asflDescription <br> هل أنت متأكد؟...</div><br>";
...
if($mode=="showList"){
    ...
    if($infoMessage!=""){
        ...
        echo "<table id='masterTable'><tr class='header'><th> الموقف </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
        $q="SELECT asflId, asflDescription FROM assetFlag ORDER BY asflId";
        $res=mysqli_query($dbc,$q);
        while($row=mysqli_fetch_assoc($res)){
            echo "<tr><td align='right'>".$row['asflDescription']."</td>
                    <td align='center' style='width: $buttonCellWidth'>
                    <form method='post'>
                    <input type='hidden' name='asflId' value='".$row['asflId']."'>

...
if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
    $q="SELECT asstDescription FROM assetStatus WHERE asstId=?";
    if($stmt=mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt, "i", $asstId)){
            if(mysqli_stmt_execute($stmt)){
                if(mysqli_stmt_bind_result($stmt, $asstDescription)){
...
if($mode=="saveedit" or $mode=="saveadd"){
    $errorMessage="";
    if($asstDescription==""){
        $errorMessage="لابد من ادخال اسم الحالة<br>";
    }
...
//*****************************************************************************************
if($mode=="saveedit"){
    $q="UPDATE assetStatus SET asstDescription=? WHERE asstId=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"si",$asstDescription,$asstId)){
            try{
                if(mysqli_stmt_execute($stmt)){
                    $infoMessage="حفظ!...";
                }    
...
//*****************************************************************************************
if($mode=="saveadd"){
    $q="INSERT INTO assetStatus(asstDescription) VALUES(?)";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"s",$asstDescription)){
            if(mysqli_stmt_execute($stmt)){
...
//*****************************************************************************************
if($mode == "delete"){
    $q="DELETE FROM assetStatus WHERE asstId=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"i",$asstId)){
            if(mysqli_stmt_execute($stmt)){
...
if($mode=="add" or $mode=="edit"){
    ...
    echo "<tr><td style='width: 100px;'>اسم الحالة:</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<input class='input-field' type='text' placeholder='اسم الحالة' maxlength='255' name='asstDescription'";
    if(isset($asstDescription)){
        echo " value='$asstDescription'";
    } 
    echo "></div></td></tr>";  

    echo "</table>";
    if($mode=='edit'){
        echo "<input type='hidden' name='asstId' value='$asstId'>";
    }
    $newMode="save$mode";
...
if($mode == "view"){
    echo "<center>";
    echo "<h3> بيانات الحالة </h3>";
    echo "<table>";
    echo "<tr><td>Description</td><td>:</td><td align='right'> $asstDescription </td></tr>";
    echo "</table>";
...
if($mode=="deleteconfirm"){
    echo "<center>";
    echo "<h3> الغاء الحالة </h3>";
    echo "<div style='text-align: center;'>سيتم الغاء $asstDescription <br> هل أنت متأكد؟...</div><br>";
...
if($mode=="showList"){
    ...
    if($infoMessage!=""){
        ...
        echo "<table id='masterTable'><tr class='header'><th> الحالة </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
        $q="SELECT asstId, asstDescription FROM assetStatus ORDER BY asstId";
        $res=mysqli_query($dbc,$q);
        while($row=mysqli_fetch_assoc($res)){
            echo "<tr><td align='right'>".$row['asstDescription']."</td>
                    <td align='center' style='width: $buttonCellWidth'>
                    <form method='post'>
                    <input type='hidden' name='asstId' value='".$row['asstId']."'>

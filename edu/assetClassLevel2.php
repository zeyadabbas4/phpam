...
if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
    $q="SELECT acl2Id, acl2Desc, acl2Level1 FROM assetClassLevel2 WHERE acl2Id=?";
    if($stmt=mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt, "s", $acl2Id)){
            if(mysqli_stmt_execute($stmt)){
                if(mysqli_stmt_bind_result($stmt, $acl2Id, $acl2Desc, $acl2Level1)){
...
if($mode=="saveedit" or $mode=="saveadd"){
    ...
    // Validate acl2Desc (it should not be empty)
    if(trim($acl2Desc) == ""){
        $errorMessage .= "لابد من ادخال اسم التكويد<br>";
    }
    
    // Validate acl2Id (it should be a valid number if it's a new record)
    if($mode == "saveadd" && (empty($acl2Id) || !is_numeric($acl2Id))){
...
//*****************************************************************************************
if($mode=="saveedit"){
    $q="UPDATE assetClassLevel2 SET acl2Desc=?, acl2Level1=? WHERE acl2Id=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"ssi",$acl2Desc,$acl2Level1,$acl2Id)){
            try{
                if(mysqli_stmt_execute($stmt)){
                    $infoMessage="حفظ!...";
                }    
...
if($mode=="saveadd"){
    // Validate input
    if(empty($acl2Desc) || empty($acl2Id) || empty($acl2Level1)) {
        $errorMessage = "لابد من إدخال اسم التكويد والكود والفئة الرئيسية.";
    }
    
    if(empty($errorMessage)){
        $q = "INSERT INTO assetClassLevel2(acl2Id, acl2Desc, acl2Level1) VALUES(?, ?, ?)";
        
        if ($stmt = mysqli_prepare($dbc, $q)) {
            // Bind acl2Id, acl2Desc, and acl2Level1
            if (mysqli_stmt_bind_param($stmt, "sss", $acl2Id, $acl2Desc, $acl2Level1)) {
                if (mysqli_stmt_execute($stmt)) {
...
//*****************************************************************************************
if($mode == "delete"){
    $q="DELETE FROM assetClassLevel2 WHERE acl2Id=?";
    if ($stmt = mysqli_prepare($dbc, $q)){
        if(mysqli_stmt_bind_param($stmt,"s",$acl2Id)){
            if(mysqli_stmt_execute($stmt)){
...
if($mode=="add" or $mode=="edit"){
    ...
    echo "<input class='input-field' type='text' placeholder='ادخل الكود' maxlength='255' name='acl2Id' value='$acl2Id'>";
    echo "</div></td></tr>";
    
    // Description field
    echo "<tr><td style='width: 100px;'>اسم التكويد:</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<input class='input-field' type='text' placeholder='الاسم ' maxlength='255' name='acl2Desc' value='$acl2Desc'>";
    echo "</div></td></tr>";
    
    // Level1 dropdown
    echo "<tr><td style='width: 100px;'>كود المستوي الاول:</td><td style='width:650px;'>";
    echo "<div class='input-container'>";
    echo "<select class='input-field' name='acl2Level1'>";
    // Query to fetch level1 options
    $q = "SELECT acl1Id, acl1Desc FROM assetClassLevel1";
    $result = mysqli_query($dbc, $q);
    while ($row = mysqli_fetch_assoc($result)) {
        $selected = ($row['acl1Id'] == $acl2Level1) ? "selected" : "";
        echo "<option value='" . $row['acl1Id'] . "' $selected>" . $row['acl1Desc'] . "</option>";
    }
    echo "</select>";
    echo "</div></td></tr>";
    ...
    echo "</table>";
    if($mode=='edit'){
        echo "<input type='hidden' name='acl2Id' value='$acl2Id'>";
    }
    $newMode="save$mode";
...
if($mode == "view"){
    ...
    echo "<table>";
    echo "<tr><td>كود التكويد</td><td>:</td><td align='right'> $acl2Id </td></tr>";
    echo "<tr><td>اسم التكويد</td><td>:</td><td align='right'> $acl2Desc </td></tr>";
    echo "<tr><td>الفئة الرئيسية</td><td>:</td><td align='right'> $acl2Level1 </td></tr>";
    echo "</table>";
...
if($mode=="deleteconfirm"){
    ...
    echo "<div style='text-align: center;'>سيتم الغاء التكويد رقم $acl2Id - $acl2Desc <br> هل أنت متأكد؟...</div><br>";
...
if($mode=="showList"){
    ...
    if($infoMessage!=""){
        ...
        echo "<table id='masterTable'><tr class='header'><th>كود المستوي التاني</th><th>الاسم</th><th>كود المستوي الاول </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
        
        $q="SELECT acl2Id, acl2Desc, acl2Level1 FROM assetClassLevel2 ORDER BY acl2Id";
        $res=mysqli_query($dbc,$q);
        while($row=mysqli_fetch_assoc($res)){
            echo "<tr>
                    <td align='right'>" . $row['acl2Id'] . "</td>
                    <td align='right'>" . $row['acl2Desc'] . "</td>
                    <td align='right'>" . $row['acl2Level1'] . "</td>
                    <td align='center' style='width: $buttonCellWidth'>
                    <form method='post'>
                    <input type='hidden' name='acl2Id' value='" . $row['acl2Id'] . "'>

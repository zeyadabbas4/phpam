...
$nomData = array();
$certTCount = 0;
$q = "select concrsId ,concrsCmp ,concrsCost ,concrsCurrency ,concrsTrnCount ,concrsTrnCerts from conCourses where concrsId=?";
if ($stmt = mysqli_prepare($dbc, $q)) {
    if (mysqli_stmt_bind_param($stmt, "i", $CoursId)) {
        if (mysqli_stmt_execute($stmt)) {
            if (mysqli_stmt_bind_result($stmt, $concrsId, $concrsCmp, $concrsCost, $concrsCurrency, $concrsTrnCount, $concrsTrnCerts)) {
                while (mysqli_stmt_fetch($stmt)) {
                    $nomData[$concrsCmp]['Cost'] = $concrsCost;
                    $nomData[$concrsCmp]['Currency'] = $currency[$concrsCurrency];
                    $nomData[$concrsCmp]['Count'] = $concrsTrnCount;
                    $nomData[$concrsCmp]['Certs'] = $concrsTrnCerts;
                    $certRowCount = $concrsTrnCerts * $concrsTrnCount;
                    $certTCount = $certTCount + $certRowCount;
                }
            }
        }
    }
...
<body>
    ...
    <div class='mainDoc'>
        ...
        <table border='1' class='top-table'>
            <tr>
                <td class='titles firstColumn'>العام التدريبي</td>
                <td><?php echo $user_year['Desc']; ?></td>
                <td class='titles thirdColumn'>المنطقه</td>
            ...
            <tr>
                ...
                <td class='titles' class='titles'>إجمالي المتدربين المنفذ</td>
                <td class='titles' class='titles'>إجمالي الشهادات</td>
                <td>&nbsp;</td>
            </tr>
            <tr>
                <td><?php echo $comps[$CoursLocation] ?></td>
                <td><?php echo $nomineesCount; ?></td>
                <td><?php echo $certTCount ?></td>
                <td>&nbsp;</td>
            </tr>
        </table>
        <h2>المتدربين</h2>
        <table class='middle-table'>
            <tr>
                ...
                <td class='titles'>العدد الفعلي</td>
                <td class='titles'>تكلفة المتدرب</td>
                <td class='titles'>العمله</td>
                <td class='titles'>الاجمالي</td>
                <td class='titles'>عدد الشهادات</td>
                <td class='titles'>إجمالي الشهادات</td>
            </tr>
            <?php foreach ($nomData as $key => $value) { ?>
                <tr>
                    <td> <?php echo $comps[$key]; ?> </td>
                    <td> <?php echo $value['Count']; ?></td>
                    <td> <?php echo $value['Cost']; ?> </td>
                    <td> <?php echo $value['Currency']; ?></td>
                    <td> <?php echo $value['Count'] * $value['Cost']; ?></td>
                    <td> <?php echo $value['Certs']; ?> </td>
                    <td> <?php echo $value['Count'] * $value['Certs']; ?></td>
                </tr>
            <?php } ?>
        </table>

        <?php include("$__includeDir/readStaff.php"); ?>
        <?php include("$__includeDir/readCourseLecs.php"); ?>
        <?php include("$__includeDir/yesNo.php"); ?>
        <h2>المحاضرين</h2>
        <table class='middle-table'>
            <tr>
            ...
            <?php foreach ($lecs as $key => $value) { ?>
                <tr>
                    ...
                    <td><?php echo $yesNo[$value['Return'] + 2]; ?></td>
                </tr>
            <?php } ?>
        </table>

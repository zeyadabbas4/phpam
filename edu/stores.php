...
if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
	$q="SELECT strdesc FROM stores WHERE strid=?";
	if($stmt=mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt, "i", $strid)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt, $strdesc)){
...
if($mode=="saveedit" or $mode=="saveadd"){
	$errorMessage="";
	if($strdesc==""){
		$errorMessage="لابد من ادخال اسم المحزن<br>";
	}
...
//*****************************************************************************************
if($mode=="saveedit"){
	$q="UPDATE stores SET strdesc=? WHERE strid=?";
	if ($stmt = mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt,"si",$strdesc,$strid)){
			try{
				if(mysqli_stmt_execute($stmt)){
					$infoMessage="حفظ!...";
				}	
...
//*****************************************************************************************
if($mode=="saveadd"){
	$q="INSERT INTO stores(strdesc) VALUES(?)";
	if ($stmt = mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt,"s",$strdesc)){
			if(mysqli_stmt_execute($stmt)){
...
//*****************************************************************************************
if($mode == "delete"){
	$q="DELETE FROM stores WHERE strid=?";
	if ($stmt = mysqli_prepare($dbc, $q)){
		if(mysqli_stmt_bind_param($stmt,"i",$strdesc)){
			if(mysqli_stmt_execute($stmt)){
...
if($mode=="add" or $mode=="edit"){
	...
	if($mode=="add"){
		$formTitle="اضف مخزن جديد";
	}else{
		$formTitle="تعديل المخزن";
	}
	...
	echo "<tr><td style='width: 100px;'>Description:</td><td style='width:650px;'>";
	echo "<div class='input-container'>";
	echo "<input class='input-field' type='text' placeholder='strdesc' maxlength='255' name='strdesc'";
	if(isset($strdesc)){
		echo " value='$strdesc'";
	} 
	echo "></div></td></tr>";  
	...
	echo "</table>";
	if($mode=='edit'){
		echo "<input type='hidden' name='strId' value='$strid'>";
	}
	$newMode="save$mode";
...
if($mode == "view"){
	echo "<center>";
	echo "<h3> بيانات المخزن </h3>";
	echo "<table>";
	echo "<tr><td>Description</td><td>:</td><td align='right'> $strdesc </td></tr>";
	echo "</table>";
...
if($mode=="deleteconfirm"){
	echo "<center>";
	echo "<h3> الغاء محزن </h3>";
	echo "<div style='text-align: center;'>سيتم الغاء $strdesc <br> هل أنت متأكد؟...</div><br>";
...
if($mode=="showList"){
	...
	if($infoMessage!=""){
		...
		echo "<button type='submit' class='addBtn' name='mode' value='add'>اضافة جديده</button>";
		echo "</form></div><br>";
		$numberOfButtons=3;
		$buttonCellWidth=$numberOfButtons * 115;
		$buttonCellWidth .= "px";
		echo "<input type='text' id='filterBox' onkeyup='filterList()' placeholder='بحث...'>";
		echo "<table id='masterTable'><tr class='header'><th> المخزن </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
		$q="SELECT strid, strdesc FROM stores ORDER BY strid";

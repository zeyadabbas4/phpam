...
//*****************************************************************************************
    if($mode=="edit" or $mode=="view" or $mode=="deleteconfirm"){
        $q="SELECT wfName FROM Authors WHERE AuthId=?";
        if($stmt=mysqli_prepare($dbc, $q)){
            if(mysqli_stmt_bind_param($stmt, "i", $AuthId)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt, $wfName)){
...
//*****************************************************************************************
	if($mode=="saveedit" or $mode=="saveadd"){
		$errorMessage="";
		if($wfName==""){
			$errorMessage="لابد من ادخال Workflow name<br>";
		}
...
//*****************************************************************************************
	if($mode=="saveedit"){
		$q="UPDATE Authors SET wfName=? WHERE AuthId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"si",$wfName,$AuthId)){
				try{
					if(mysqli_stmt_execute($stmt)){
						$infoMessage="Saved!...";
					}	
...
//*****************************************************************************************
	if($mode=="saveadd"){
		$q="INSERT INTO Authors(wfName) VALUES(?)";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"s",$wfName)){
				if(mysqli_stmt_execute($stmt)){
					$infoMessage="تم الSave!...";
				}else{
...
//*****************************************************************************************
	if($mode == "delete"){
		$q="DELETE FROM Authors WHERE AuthId=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_bind_param($stmt,"i",$AuthId)){
				if(mysqli_stmt_execute($stmt)){
...
//*****************************************************************************************
	if($mode=="add" or $mode=="edit"){
		...
		echo "<tr><td style='width: 100px;'>Workflow name:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<input class='input-field' type='text' placeholder='Workflow name' maxlength='255' name='wfName'";
		if(isset($wfName)){
			echo " value='$wfName'";
		} 
		echo "></div></td></tr>";  

		echo "<tr><td style='width: 100px;'>Country:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<select class='input-field' name='country'><optio value='-1'>Select country</option>";
		$q="SELECT cntId,cntName FROM countries ORDER BY cntName";
		if($stmt=mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt, $cntId,$cntName)){
		...
		echo "<tr><td style='width: 100px;'>Enabled:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<input type='hidden' name='enabled' value='0'>";
		echo "<input class='input-field' type='checkbox' name='enabled' value='1'";
		if(isset($enabled)){
			if($enabled=='1'){
				echo " checked";	
			}
		...
		echo "</table>";
		if($mode=='edit'){
			echo "<input type='hidden' name='AuthId' value='$AuthId'>";
		}
		$newMode="save$mode";
...
//*****************************************************************************************
	if($mode == "view"){
		echo "<center>";
		echo "<h3> بيانات المؤلف </h3>";
		echo "<table>";
		echo "<tr><td>Workflow name</td><td>:</td><td align='right'> $wfName </td></tr>";
		echo "</table>";
...
//*****************************************************************************************
	if($mode=="deleteconfirm"){
		echo "<center>";
		echo "<h3> الغاء مؤلف </h3>";
		echo "<div style='text-align: center;'>سيتم الغاء $wfName <br> هل أنت متأكد؟...</div><br>";
...
//****************************************************************************************
	if($mode=="showList"){
		...
		$q="SELECT sotId,sotStaffId,staffname FROM staffOvertime INNER JOIN staff ON sotId=staffid WHERE staffid=?";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt, $AuthId,$wfName)){
					while(mysqli_stmt_fetch($stmt)){
						echo "<tr>";
						echo "<td> $AuthId - $wfName </td>";

...
if(isset($_SESSION['__uid'])){
  ...
  if($logged){
    ...
    $pageTitle="دورات المحاكي لعام ".$user_year['Desc'];
    $pageSubTitle="مكتب العميد - إعتماد نتيجة دورات المحاكي";
    ...
    echo ".navBtn {background-color: dodgerblue;  color: white;  padding: 10px 10px;  border: none;  cursor: pointer;  width: 200px;  opacity: 0.9;}";
...
//************************************************************************************ */    
    if($mode=='view' ){
      ...
      echo "<button type='submit' name='mode' value='agreeConfirm' class='yesBtn' style='width: 150px;$hide'>إعتماد الدورة</button> "; 
      echo "<button type='submit' name='mode' value='rejectConfirm' class='noBtn' style='width: 200px;$hide'>رد ﻹدارة الموارد البشرية</button> ";  
...
/*********************************************************************************************************** */
    if($showList){
      ...
      $q="SELECT CoursId,CoursBulletin,CrsName,CoursStatus FROM Courses inner join CoursesGuide on CoursCrsId=CrsId WHERE CoursType=$CoursType and CoursYear=".$user_year['Id']." and CoursStatus >=$formCourseStatusLevel order by CoursStatus,CoursBulletin";
      ...
      if($r){
        ...
        echo "<th>المنشور - اسم الدورة</th><th style='width:120px;text-align: center;'></th></tr>";
        $rowNo=0;
        while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
          ...
          $displayButtonText="مراجعة";
          if($CoursStatus>$formCourseStatusLevel){
            $approved=" [تم إعتماد الدورة] ";
            $displayButtonClass="grpBtn";
            $displayButtonText="عرض";
          }
          
          echo "<tr><td>$CoursBulletin - $CrsName $approved</td>";
          echo "<td style='text-align: right;'><form method='post'>";
          echo "<input type='hidden' name='CoursId' value='$CoursId'>";
          echo "<button type='submit' class='$displayButtonClass' name='mode' value='view'>$displayButtonText</button> ";
          echo "</form></td></tr>";
        }
        echo "</table>";

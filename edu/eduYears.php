...
//*****************************************************************************************
	if($mode=="saveadd"){
		$q="INSERT INTO  Years(YearDesc,YearStart,YearEnd,YearType) VALUES(?,?,?,'E')";
		if ($stmt = mysqli_prepare($dbc, $q)){
...
//*****************************************************************************************
	if($mode=="add" or $mode=="edit"){
		...
		echo "<tr><td style='width: 100px;'>اسم العام التدريبي:</td><td style='width:650px;'>";
		echo "<div class='input-container'>";
		echo "<input class='input-field' type='text' placeholder='اسم العام التدريبي' maxlength='255' name='YearDesc'";
		if(isset($YearDesc)){
			echo " value='$YearDesc'";
		} 
		echo "></div></td></tr>";  
...
//*****************************************************************************************
	if($mode == "view"){
		$showList=false;
		echo "<center>";
		echo "<h2 style='text-align: center;'> بيانات العام التدريبي </h2>";
		echo "<table>";
		echo "<tr><td>العام التدريبي</td><td>:</td><td align='right'> $YearDesc </td></tr>";
...
//*****************************************************************************************
	if($mode=="deleteconfirm"){
		$showList=false;
		echo "<center>";
		echo "<h2 style='text-align: center;'> الغاء عام تدريبي </h2>";
...
//*****************************************************************************************
	if($mode=="current"){
		setSystemValue("PreEduYear",$currentEduYear);
		setSystemValue("curEduYear",$YearId);
		$currentEduYear=getSystemValue('curEduYear');
		$previousEduYear=getSystemValue('PreEduYear');
	}
...
//****************************************************************************************
	if($showList){
		...
		echo "<button type='submit' class='addBtn' name='mode' value='add' style='width: 150px;'>إضافة عام جديدة</button>";
		echo "</form></div><br>";
		$numberOfButtons=4;
		$buttonCellWidth=$numberOfButtons * 115;
		$buttonCellWidth .= "px";
		echo "<center>Current year: " . $currentEduYear . " Previous year:" .$previousEduYear ."</center>";
		//filter list form 
		echo "<input type='text' id='filterBox' onkeyup='filterList()' placeholder='بحث...'>";
		echo "<table id='masterTable'><tr class='header'><th> العام التدريبي </th><th style='width: $buttonCellWidth'>&nbsp;</th></tr>";
		$q="SELECT YearId,YearDesc FROM  Years WHERE YearType='E' ORDER BY YearDesc";
		if ($stmt = mysqli_prepare($dbc, $q)){
			if(mysqli_stmt_execute($stmt)){
				if(mysqli_stmt_bind_result($stmt, $YearId,$YearDesc)){
					while(mysqli_stmt_fetch($stmt)){
						$currentButtonStatus="";
						$currentButtonStyle="";
						if($YearId == $currentEduYear){
							$currentButtonStatus="disabled";
							$currentButtonStyle=" background-color: grey;";
						}
						echo "<tr>";
						...
						echo "<input type='hidden' name='YearId' value='$YearId'>";
						echo "<button type='submit' class='arrowBtn' name='mode' value='current' style='width:100px;$currentButtonStyle' $currentButtonStatus>الحالي</button> ";

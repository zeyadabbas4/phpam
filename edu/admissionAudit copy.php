...
if(isset($_SESSION['__uid'])){
  ...
  if($logged){
    ...
    $pageSubTitle="مراجعة القبول والتسجيل";
...
/*********************************************************************************************************** */
    if($showList){
      ...
      $q="SELECT CoursId,CoursBulletin,CrsName,CoursStatus FROM Courses inner join CoursesGuide on CoursCrsId= CrsId WHERE CoursType=1 and CoursYear=".$user_year['Id']." and CoursStatus >1 order by CoursSection,CoursBulletin ";
      $r=mysqli_query($dbc,$q);
      if($r){
        //filter list form 

...
if(isset($mode)){
    ...
    if($errorMessage==""){
        ...
        $q="SELECT CoursId,CoursBulletin,CrsName,CoursFromAct,CoursToAct,CoursFromPln,CouursToPln,dist_name,dist_id,PrgName,PrgId,crscmpPlanned,cmpName,cmpId FROM Courses INNER JOIN CoursesGuide ON CoursCrsId=CrsId INNER JOIN districts ON CoursArea=dist_id INNER JOIN Programs ON CrsProgram=PrgId INNER JOIN courseCompanies ON CoursId=crscmpCourse INNER JOIN companies ON crscmpCompany=cmpId WHERE CoursType=1 AND CoursFromPln>? AND CouursToPln<? $compFilter $distFilter $progFilter ORDER BY dist_name,cmpName,PrgName,CoursFromPln";
        if ($stmt = mysqli_prepare($dbc, $q)){
            if(mysqli_stmt_bind_param($stmt,$types,...$parameters)){
                if(mysqli_stmt_execute($stmt)){
                    if(mysqli_stmt_bind_result($stmt,$CoursId,$CoursBulletin,$CrsName,$CoursFromAct,$CoursToAct,$CoursFromPln,$CouursToPln,$dist_name,$dist_id,$PrgName,$PrgId,$crscmpPlanned,$cmpName,$cmpId)){
                    ...
                    if(mysqli_stmt_execute($stmt)){
                        if(mysqli_stmt_bind_result($stmt,$trnCountAct)){
                            if(mysqli_stmt_fetch($stmt)){
                                ...
                                $result[$key]['trnCountDiff']=$trnCountAct-$crscmpPlanned;
                            }
                        }
                    }
                }
        ...
        <div class='mainDoc'>
            ...
            echo "<table class='contentTable'>
                <tr>
                    ...
                    <th rowspan='2'>رقم المنشور</th>
                    <th rowspan='2'>إسم الدورة</th>
                    <th rowspan='2'>المنطقة</th>
                    <th colspan='2'>تاريخ الدورة (مخطط)</th>
                    <th colspan='2'>تاريخ الدورة (فعلي)</th>
                    <th colspan='3'>عدد المتدربين</th>
                </tr>
                <tr>
                    <th>من</th>
                    <th>إلى</th>
        ...
        foreach($result as $key => $value){
            ...
            echo "<tr>
                    ...
                    <td>".$value['trnCountDiff']."</td>
                </tr>";
            $totPlaned+=$value['crscmpPlanned'];
            $totActual+=$value['trnCountAct'];    
        }
        $totDiff=$totActual-$totPlaned;
        echo "<tr><td colspan='8' >إجمالي</td><td>$totPlaned</td><td>$totActual</td><td>$totDiff</td></tr>";
        echo "
            </table>";
        echo "</div>";
    }

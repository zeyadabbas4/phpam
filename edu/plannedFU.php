...
if(isset($_SESSION['__uid'])){
  ...
  if($logged){
    ...
    $pageTitle="متابعة الدورات المخططة لعام ".$user_year['Desc'];
    echo "<!DOCTYPE html><html><head>";
    echo "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">";
    echo "<title>$pageTitle</title>";
    echo "<style>";
    echo "*{ box-sizing: border-box;}";
    echo "#filterBox {background-image: url('img/searchicon.png');background-position: 10px 10px;background-repeat: no-repeat;width: 100%;font-size: 16px;padding: 12px 20px 12px 40px;border: 1px solid #ddd;margin-bottom: 12px;}";
...
//************************************************************************************************************* */
    if($mode == "register"){
      ...
      //read trainees
      $trn=array();
      $q="SELECT cmpName,TrnName,TrnNo,TrnIdNo FROM ((Courses inner join coursetrainees on crstrncourse=CoursId) inner join Trainees on TrnNo = crstrntrainee) inner join companies on cmpId = crstrncompany WHERE CoursId=? order by cmpId";
      if ($stmt = mysqli_prepare($dbc, $q)) {
        if(mysqli_stmt_bind_param($stmt, "i", $CoursId)){
          if(mysqli_stmt_execute($stmt)){
      ...
      echo "<br><br><h3>اﻷعداد المسجلة</h3>";
      echo "<table style='width:50%;margin:auto;border-collapse: collapse;border: 1px solid black;'>";
      echo "<tr><td style='border: 1px solid black;'> م </td><td style='border: 1px solid black;'> الشركة </td><td style='border: 1px solid black;'> المخطط </td><td style='border: 1px solid black;'> المسجل </td><td style='border: 1px solid black;'> نقص/زيادة </td><tr>";
      foreach($nom as $key  => $value){
        $count=$key+1;
        ...
        echo "<tr><td style='border: 1px solid black;'> $count </td><td style='border: 1px solid black;'> $company </td><td style='border: 1px solid black;'> $planned </td><td style='border: 1px solid black;'>$actual</td><td style='border: 1px solid black;'>$difference</td><tr>";
      }
      echo "</table>";
...
//************************************************************************************************************* */
   if($mode == "lecs"){
      ...
      $tot=$totT+$totP;
      echo "<tr>";
      echo "<td width='300px' style='text-align:right;font-weight:bold;' colspan='2'>إجمالي الساعات</td>";
...
//************************************************************************************************************* */
    if($showList){
      ...
      $q="SELECT CoursId,CoursBulletin,CrsName,CoursStatus FROM Courses inner join CoursesGuide on CoursCrsId= CrsId WHERE CoursType=1 and CoursStatus >= $formCourseStatusLevel-1 and CoursYear=".$user_year['Id']." order by CoursStatus,CoursBulletin";
      $r=mysqli_query($dbc,$q);

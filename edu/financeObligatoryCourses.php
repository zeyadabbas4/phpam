...
if(isset($_SESSION['__uid'])){
  ...
  if($logged){
    ...
    echo ".badge {background-color: red;color: white;padding: 4px 8px;text-align: center;border-radius: 5px;font-size: x-small;position:relative;top:-5;}";    echo "body{direction:rtl;}";
    echo "</style>";
    echo "</head>";
	  echo "<body>";
...
/*********************************************************************************************************** */
    if($showList){
      ...
      if(isset($errorMsg)){
        echo "<div class='infoMsg'>$errorMsg</div>";
        $errorMsg="";
      }
      $q="SELECT CoursId,CoursBulletin,CrsName,CoursStatus,CoursPrtBenifits,CoursPrtSupervision,CoursPrtCourseData FROM Courses inner join CoursesGuide on CoursCrsId=CrsId WHERE CoursType=4 and CoursYear=".$user_year['Id']." and CoursStatus >=$formCourseStatusLevel order by CoursStatus,CoursBulletin";

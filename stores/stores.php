...
if(isset($_SESSION['__uid'])){
  ...
  if($logged){
    ...
    if($mode==5){
      //حفظ new
      if($strdesc==""){
        $errorMessage.="لا بد من ادخال اسم المخزن<br>";
        $mode=1;
      }
      if($mode==5){
        $q="insert into stores(strdesc) values(?)";
        if($stmt = mysqli_prepare($dbc, $q)){
          mysqli_stmt_bind_param($stmt, "s", $strdesc);
    ...
    if($mode==1){
      ...
      echo "<tr><td style='width: 225px;'>اسم المخزن:</td><td colspan='2'>";
      echo "<div class='input-container'>";
      echo "<input class='input-field' type='text' placeholder='اسم المخزن' name='strdesc'";
      if(isset($strdesc))
        echo " value='$strdesc'";
      echo ">";
      echo "</div></td></tr>";
    ...
    if($mode==6){
      ...
      if($mode==6){
        ...
        if ($stmt = mysqli_prepare($dbc, $q)){
          mysqli_stmt_bind_param($stmt, "si", $strdesc,$strid);
          mysqli_stmt_execute($stmt);
          mysqli_stmt_close($stmt);
        }
      }
    ...
    if($mode==2){
      ...
      if($errorMessage ==""){
        ...
        if ($stmt = mysqli_prepare($dbc, $q)) {
          mysqli_stmt_bind_param($stmt, "i", $strid);
          mysqli_stmt_execute($stmt);
          mysqli_stmt_bind_result($stmt, $strdesc);
    ...
    if($mode==4){
      ...
      echo "<div style='text-align: center;direction:$__dir;'>سيتم الغاء $strdesc هل انت متأكد?...</div><br>";
      echo "<input type='hidden' name='strid' value='$strid'>";
    ...
    if($mode==3){
      ...
      echo "<table width='100%'>";
      echo "<tr><td style='width: 225px;'>اسم المخزن:</td><td>";
      echo "<div class='input-container'>$strdesc</div></td></tr>";
      echo "</table>";
    ...
    if($showList){
      $q="select * from stores order by strdesc";
      ...
      if($r){
        ...
        echo "<th>اسم المخزن</th><th style='width:600px;text-align: center;'></th></tr>";
        while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
          foreach($row as $key => $value)
            $$key=$value;
          echo "<tr><td>$strdesc</td>";

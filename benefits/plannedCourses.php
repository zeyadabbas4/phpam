...
//************************************************************************************ */    
    if($mode=='view' ){
      ...
      $financeButtonhide="display: none;";
      ...
      echo "<button type='submit' name='mode' value='agreeConfirm' class='yesBtn' style='width: 150px;$hide'>اﻹعتماد من العميد</button> "; 
      echo "<button type='submit' name='mode' value='rejectConfirm' class='noBtn' style='width: 150px;$hide'>رد ﻹدارة التخطيط</button> ";  
      echo "<button type='submit' name='mode' value='sendFinanceConfirm' class='edtBtn' style='width: 150px;$financeButtonhide'>ارسال للادارةالمالية</button> ";  
...
************************************************************************************/
    ...
    if($mode=="sendFinanceSave"){
      $newFlag=$formCourseStatusLevel+3;
      $mode="Save";
    }
...
if($mode=="printBenefits"){
  ...
  echo "
  <script>
  window.open(\"courseBenefits.php\");
  </script>
  ";
}
...
if($mode=="printCourseData"){
  ...
  echo "
  <script>
  window.open(\"courseLecturerData.php\");
  </script>
  ";
}
    ...
    if($showList){
      ...
      if($r){
        ...
        while($row=mysqli_fetch_array($r,MYSQLI_ASSOC)){
          ...
          if($CoursStatus >= $formCourseStatusLevel+2){       //When course is abroved by the dean
            $disabled="";
          }
          $displayButtonClass="viewBtn";
          if($CoursStatus == $formCourseStatusLevel+1){
            $displayButtonClass="grpBtn";
          }elseif($CoursStatus == $formCourseStatusLevel+2){
            $displayButtonClass="delBtn";
          }elseif($CoursStatus >= $formCourseStatusLevel+3){
            $displayButtonClass="addBtn";
          }
          $rowNo++;
          echo "<tr><td>$CoursBulletin - $CrsName</td>";
          echo "<td style='text-align: right;'><form method='post'>";
          echo "<input type='hidden' name='CoursId' value='$CoursId'>";
          echo "<button type='submit' class='$displayButtonClass' name='mode' value='view'>عرض</button> ";
          echo "<button type='submit' class='navBtn' name='mode' value='printBenefits'$disabled>المستحقات</button> ";
          echo "<button type='submit' class='navBtn' name='mode' value='printSupervision'$disabled>الاشراف</button> ";
          echo "<button type='submit' class='navBtn' name='mode' value='printCourseData'$disabled>بيانات الدورة</button> ";
          echo "</form></td></tr>";
        }
        echo "</table>";
